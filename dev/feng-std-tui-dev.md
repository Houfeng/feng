...
- [ ] 4.18 实现事件类型：`KeyEvent.ff`（`SpecialKey` 枚举、`MOD_CTRL`/`MOD_SHIFT` 常量、`Union<SpecialKey,u32>`、`KeyEvent` @value 类型 + `isCtrl()`/`isShift()`/`isPrintable()` 快捷方法）和 `MouseEvent.ff`（`MouseAction`/`MouseButton` 枚举、`MouseEvent` @value 类型 + 快捷方法）
