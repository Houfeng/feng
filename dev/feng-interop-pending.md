...
2. `@abi` 参与 ABI 边界时，传值/传指针规则由签名决定（`T` 或 `T*`）。
3. `@abi` 相关限制继续严格执行：
...
2. `string`
3. ABI 兼容数组
4. 标注 `@abi` 且通过 ABI 检查的顶层 `fn`（目标类型必须显式给出 `Foo*`）
...
4. `&abi_fn`：在目标类型显式给出为 `Foo*` 时，返回该 `@abi` 顶层 `fn` 的函数指针。
...
1. `string` 在 ABI 边界只定义借用与 0 拷贝能力；具体 ABI 形状由 `extern fn` 签名显式表达，语言不预设未知 C API。
...
如果某个 C API 明确要求可写 `char*` 缓冲，且开发者已在 `extern fn` 签名中如此声明，可直接使用 `string` 借用；若 C 写入破坏字符串约束（如 UTF-8/长度一致性等），行为未定义（UB，出错后不保证任何行为）。
...
- [ ] T4.1 落实“默认借用，仅保证调用期间有效”的语义检查。
- [ ] T4.2 编译器不做自动延寿；需要跨调用持有时，由开发者自行保活 owner。
- [ ] T4.3 补借用诊断：明显逃逸误用、缺少保活责任提示、错误的 `Foo*` 来源说明。
