...
`FengTypeDescriptor.agg_deps[]` 和 `type_deps[]` 的索引在整个类型范围内**全局稳定**：codegen 收集该类型所有方法中全部依赖（成员字段 + 各方法体局部依赖），按依赖排序 key（见 §2.2）字典序升序分配全局唯一索引，所有方法共享同一 `FengTypeDescriptor`，各方法按编译期确定的固定索引访问各自所需的描述符。

**路径二：独立泛型函数（无 `self`）**

Wrapper 为本次具体化静态生成一个 **`FengFunctionDescriptor`**（`static const`），传入共享体时命名为 `_desc`。共享体通过 `_desc->agg_deps[i]` / `->type_deps[i]` 在运行时访问各依赖。

两条路径形式完全统一：`自身具体化描述符->agg_deps[i]` / `->type_deps[i]`，区别仅在"自身具体化描述符"的来源。
...
- 每个参数 `pi` 递归以**参数模式**展开；`UserType` 的泛型参数名（如 `K`、`V`）直接使用，不替换

示例（`UserType<K,V>` 内，依赖 `Foo<int,K>`、`Bar<V>`、`Xyz<Foo<int,K>,Bar<V>>`）：

| 类型 | 排序 key（根模式） |
|------|------|
| `Foo<int,K>` | `Foo__int__K` |
| `Bar<V>` | `Bar__V` |
| `Xyz<Foo<int,K>,Bar<V>>` | `Xyz__Foo_int_K__Bar_V` |

`Foo<int,K>` 作为 `Xyz` 的参数时展开为参数模式 `Foo_int_K`；`Bar<V>` 展开为 `Bar_V`。

三者按字典序：`Bar__V` < `Foo__int__K` < `Xyz__Foo_int_K__Bar_V`，分别占 `agg_deps[0]`、`agg_deps[1]`、`agg_deps[2]`。
 ...
 * agg_deps 按全局稳定顺序（类型名 + 泛型参数顺序）覆盖该类型所有方法的 aggregate 依赖 */
...
`_K`、`_V` 保留，用于直接泛型参数字段的 retain/release 和大小（基础类型 kind/size）。共享体通过 `_desc->agg_deps[i]` / `->type_deps[i]` 在运行时访问各依赖描述符。

**泛型类型的方法共享体**不新增任何描述符参数，所有依赖通过 `self->_hdr.desc->agg_deps[i]` / `->type_deps[i]` 访问，`self->_hdr.desc` 已是该次调用的具体化 `FengTypeDescriptor`。
...
```c
/* 独立函数：_desc->agg_deps[0] 是 MapEntity<K,V> 的具体化描述符 */
const FengAggregateDescriptor *_ed = _desc->agg_deps[0];
FengArray *_arr = feng_array_new_kinded(
    FENG_VALUE_AGGREGATE_WITH_MANAGED_SLOTS, _ed, NULL, _ed->size, _n);

/* 类型方法：通过 self->_hdr.desc->agg_deps[i]，大小在运行时读取 */
...
/* 独立函数场景 */
const FengAggregateDescriptor *_ed = _desc->agg_deps[0];
_Alignas(max_align_t) char _tmem[_ed->size];  /* VLA，大小在运行时从具体化描述符读取 */
feng_aggregate_default_init(_tmem, _ed);
struct MapEntity__G__K__V *_tuple = (struct MapEntity__G__K__V *)_tmem;

/* 类型方法场景：操作成员字段 entry（Foo<int,V> 是 tuple，agg_deps[i] 按全局稳定索引） */
const FengAggregateDescriptor *_entry_desc = self->_hdr.desc->agg_deps[1];
feng_aggregate_assign(&self->entry, &new_entry, _entry_desc);
```
...
无需额外 `_type_desc` 参数，所有具体化类型描述符均由 `type_deps[]` 提供。
...
| `src/codegen/codegen.c` | Wrapper 生成具体化描述符树（`FengTypeDescriptor.agg_deps`/`type_deps` 按排序 key 全局稳定）；独立函数共享体 ABI 增加 `const FengFunctionDescriptor *_desc` 参数；类型方法共享体不增加参数；修复 6 个创建路径（§2.5.1–§2.5.6）；读取 ft ATTRS 中 `GENERIC_AGG_DEP`/`GENERIC_TYPE_DEP` 支持跨包特化 |
