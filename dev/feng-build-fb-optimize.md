...
- [ ] 1.1 在 `FengCliProjectManifest` 中新增 assets 映射结构（目标目录 -> 源路径）。
- [ ] 1.2 扩展 `manifest.c` 解析逻辑，支持 `[assets]` 节并校验：
	- key 非空（目标目录）。
	- value 非空（相对 `feng.fm` 目录路径）。
	- 禁止与现有字段冲突（节内重复 key 报错）。
- [ ] 1.3 扩展 manifest 写回逻辑，按稳定顺序输出 `[assets]`。
- [ ] 1.4 错误信息统一：包含文件、行号、字段名。
...
- [ ] 2.1 统一路径解析基准：`[assets]` 的 value 一律相对 `feng.fm` 所在目录。
- [ ] 2.2 `target=bin`：将每条 assets 映射复制到可执行文件同级目标目录。
- [ ] 2.3 `target=lib`：将每条 assets 映射复制到打包 staging（用于后续写入 `.fb`）。
- [ ] 2.4 复制策略明确：
	- 目录递归复制。
	- 保持相对层级。
	- 冲突时直接报错（不静默覆盖）。
...
- [ ] 3.1 扩展 `FengFbLibraryBundleSpec`，增加可选目录输入：
	- `extlib_root`（按平台分发目录）
	- `assets_entries`（目标目录映射）
- [ ] 3.2 `fb.c` 写包时新增目录写入：
	- `extlib/<platform>/...`（若存在，允许同时包含静态库与动态库）
	- `[assets]` 指定的目标目录树
- [ ] 3.3 保持现有 `mod/` + `lib/` 写入逻辑回归测试通过。
...
- [ ] 4.1 `target=bin` 由 `feng` 顶层命令执行释放逻辑，只扫描 `extlib/<platform>/` 下动态库后缀：
	- Linux: `.so`
	- macOS: `.dylib`
	- Windows: `.dll`
- [ ] 4.2 释放目标固定为可执行文件同级目录。
- [ ] 4.3 不释放 `lib/` 下 feng 正式静态库；`extlib` 中静态库（`.a/.lib`）只链接不释放。
...
- [ ] 5.2 禁止新增隐式扫描来源（磁盘全盘扫描、未声明目录自动注入）。
- [ ] 5.3 `extlib` 中静态库若参与链接，必须通过显式规则接入（例如 `extern fn` 元信息），不做目录自动注入。
...
- [ ] 6.1 Manifest 单测：`[assets]` 合法/非法解析。
- [ ] 6.2 Build 集成：
	- `target=bin` 复制 assets 到同级目标目录。
	- `target=lib` 将 assets 打进 `.fb`。
- [ ] 6.3 Bundle 集成：`extlib` + assets 目录写包与解包校验。
- [ ] 6.4 Runtime 集成：验证 `extlib` 静态库只链接不释放，且仅 `extlib` 动态库触发释放。
- [ ] 6.5 回归：现有 `pack/deps/build` 主链通过。
