...
> 状态：待人工 Review。本文只收敛将编译器发行包顶层 `pkg/` 目录接入
...
- [ ] 0. 等待本文人工 Review；Review 通过后再开始实现。
- [ ] 1. 更新依赖管理主规范和发行目录规范。
- [ ] 2. 在发行组装与校验中加入通用 `pkg/` 目录。
- [ ] 3. 在精确版本安装入口中加入无 registry 时的 `pkg/` 安装。
- [ ] 4. 加入 registry 包不存在时的 `pkg/` 回退。
- [ ] 5. 复用既有校验和原子写入能力，将随附包安装到全局 cache。
- [ ] 6. 新增 install、递归依赖、build 和发行组装测试。
- [ ] 7. 在 Codex 沙箱外执行全量 `make test`。
- [ ] 8. 整理实现与测试结果，等待交付 Review。
