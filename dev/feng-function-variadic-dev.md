...
- [ ] **S1**：`src/parser/analyzer.c`，`function_type_parameters_match_args()`（line ~7713-7740）  
...
- [ ] **S2**：`src/parser/analyzer.c`，`callable_parameters_match_args()`（line ~8440-8470）  
...
- [ ] **C2**：`src/codegen/codegen.c`，通过 spec 值调用路径  
...
- [ ] **T6**：callable-form spec 变参声明，lambda 实现，通过 spec 值调用
  ```feng
