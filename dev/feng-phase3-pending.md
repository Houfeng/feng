...
- 为 `.fi` 生成预留章节与落点，后续再细化。

### 2.2 本阶段实现子集

- 第三阶段先不处理外部包，因此不实现 `deps add/remove/install`，不做本地缓存，不消费外部 `.fb`。
- `build` 只处理当前项目 `feng.fm`，不展开外部依赖图。
- 当前阶段讨论的“项目 `feng.fm`”与分发包内的 `feng.fm` 不是同一职责：项目级命令读取开发态 `feng.fm` 以组装构建参数；后续编译器在消费 `.fb` 时，仍可读取包内 `feng.fm` 的分发元信息（如 CPU 架构、ABI、支持平台等）。
...
3. 建立 `.fb` 打包 staging 目录，明确 `feng.fm`、`mod/`、`lib/` 等内容的来源与落点；其中包内 `feng.fm` 视为分发元信息，供后续编译器读取平台/架构等包能力信息，不再等同于开发态项目清单。
...
- `.fi` 生成需要单独讨论，本文件先预留章节，不和前面工作混在一起。
...
3. 第三批：`.fi` 生成细化 + 项目级 smoke + 全量回归补齐。
