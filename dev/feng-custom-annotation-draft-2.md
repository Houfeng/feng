...
自定义注解在 pre-Parse 跑，无符号表，**只能接受 Token 流参数，无法解析常量绑定的值**——标识符透传为 Token，handler 可当名字用，但不能解析成值。内建语义注解在语义阶段跑，有符号表，可接受字面量及常量绑定（解析为值）。

这与"内建是语义注解、自定义是语法注解"的边界一致：

- 语义注解（`@abi` 等）依赖符号表——自定义注解本来就不能实现这类。
- 语法注解用途上 Token 参数通常够用（`@platform(target == "linux")` 里 `target` 是 build 变量名、`"linux"` 是字面量；`@async(timeout = 1000)` 里 `1000` 是字面量）。
