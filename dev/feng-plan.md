...
- [x] **第六阶段**：细化 C ABI 兼容和互操作。把 `clib` 选择、`extern func`、`@fixed`、调用约定、头文件生成、`string` 和数组桥接、异常不得越过 ABI 边界、C 侧长期持有对象的 retain/release 协议做成完整实现。这个阶段之前，不建议主动扩大 ABI 兼容集合。
...
- C 后端更贴合当前 `@fixed`、`extern func`、包产物与头文件生成方向。
...
- 支持“本地源码 + 外部 `.fb` 包”混编。
- 以 Feng 包消费闭环为边界收口第四阶段，不在本阶段扩展 C 互操作面。
...
- 本地源码可以引用外部 `.fb`。
- 现有本地源码路径不回退。
- 当前该阶段所定义范围已完成；依赖管理与构建工具侧包解析移入 Phase 5，`clib` 选择与 C 侧互操作相关能力移入 Phase 6。

### Phase 5：包管理机制

目标：

- 建立围绕 `feng.fm` 的依赖管理与构建工具包解析机制。

范围：

- `[dependencies]` 的本地路径 / 远程版本规则
- `[registry]` 与全局配置
...
- Phase 3 先解决“本地项目能打包”，并把 `.ft` 生成放到该阶段最后处理；Phase 4 再解决“显式消费外部包”；Phase 5 再解决“依赖管理与构建工具展平”。
...
- 检查生成的 `feng.fm`、`mod`、`lib` 布局是否符合规范；`clib` / `include` 相关互操作资产放到 Phase 6 再统一验收。

### 6.5 阶段 4

- 至少准备一个“本地 app + 一个本地生成 `.fb` 包”的集成测试。
- 验证显式 `--pkg` 输入下的 `import`、`.ft` 解析、`lib` 链接顺序与冲突诊断。

### 6.6 阶段 5

- 增加本地路径依赖、远程缓存依赖与依赖图展平的构建集成测试。
- 增加 `feng deps add` / `remove` / `install` 的行为回归。
- 增加本地路径递归构建、循环依赖与版本冲突诊断测试。

### 6.7 阶段 6

- 增加 `clib` 选择与头文件协同的集成测试。
- 增加 C 宿主调用 Feng 导出函数测试。
- 增加 Feng 调用 `extern func` 测试。
...
- 第三阶段先解决“本地项目能打包”。
- 第四阶段已完成“显式外部 `.fb` 消费”。
- 第五阶段已完成依赖管理机制与构建工具依赖展平。
- `clib` 选择与更完整的 C 互操作集中放到第六阶段。
- 第六阶段之前不主动扩大 C ABI 兼容集合，也不承诺完整的 `string` / 数组桥接协议。
- 第七阶段标准库建立在已经稳定的运行时、构建和包机制之上。
