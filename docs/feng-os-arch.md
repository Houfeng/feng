...
> 其他文档（[feng-package.md](./feng-package.md) 的包目录、[feng-cli.md](./feng-cli.md) 的 `--platform` 参数、[feng-build.md](./feng-build.md) 的工具链转换、[feng-std-platform.md](./feng-std-platform.md) 的 API 行为、[../dev/feng-release-and-instanll.md](../dev/feng-release-and-instanll.md) 的分发物命名）仅引用本规范，不重复定义值域。
