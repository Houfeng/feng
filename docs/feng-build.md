...
- **编译器（`feng` 直接模式）**: 接受源文件列表和包路径列表,负责类型检查、代码生成和链接参数收集; 对 `--pkg` 指定的 `.fb`,编译器主要直接读取其中的 `.fi` 与所需库文件,必要时也可读取包内 `feng.fm` 的分发元信息; 编译器不读取项目 `feng.fm`。
- **构建工具（`feng build`）**: 读取项目 `feng.fm`,解析依赖图,按精确版本检查并安装依赖,将依赖图展平为编译器可直接消费的 `--pkg <.fb路径>` 列表,再组装其他编译参数。

两者职责分离的核心原因:编译器只需要足够的信息完成一次编译,不应被项目清单格式绑定; 但包内 `feng.fm` 属于 `.fb` 分发格式本身的一部分,因此可以作为编译器消费包时的元信息来源。构建工具可替换,编译器可被第三方构建系统直接驱动。
...
编译器不接受包名、版本号或搜索路径,不读取项目 `feng.fm`。对 `--pkg` 指定的 `.fb`,编译器主要直接读取其中的 `.fi` 与对应平台库文件; 在需要判断 `arch`、`abi` 等分发元信息时,也可直接读取包内 `feng.fm`。当前版本不支持交叉编译,始终以当前平台为目标平台。
...
编译器根据 `.fi` 中的声明关键字、`@fixed` / `extern` 元信息以及 `.fb` 内 `feng.fm` 的 `abi` / `arch` 字段，自动确定当前平台可用的链接目标:
...
构建工具传入的是**已确定并展平的 `.fb` 路径列表**,不传包名、版本或搜索路径。编译器只认路径,并直接从这些 `.fb` 中读取所需 `.fi`、库文件以及必要的包内 `feng.fm` 元信息。
...
feng src/*.ff --pkg a.fb --pkg b.fb
    │
    ├─ 扫描 .fb，必要时读取包内 feng.fm，建模块索引
    ├─ 处理 use，定位 .fi，类型检查
    ├─ 收集 @cdecl 注解，生成链接参数
    ├─ 从 .fb 读取当前平台所需 lib/clib，生成链接参数
    └─ 调用 C 链接器，产出最终二进制
```
