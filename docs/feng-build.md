...
- **编译器（`feng` 直接模式）**: 接受源文件列表和已平铺的包路径列表（`--pkg <.fb路径>`）,负责类型检查、代码生成和链接参数收集。编译器不读取任何 `feng.fm`,也不解析依赖树。
...
- 编译器不定义额外的运行时动态库发现机制。
- 对 `feng build` 且 `target=bin` 的场景,构建工具应将依赖树中 `.fb` 包提供的当前平台动态库收集并释放到可执行文件目录。
- 除上述构建场景外,部署阶段需要保证动态库位于可执行文件目录或系统加载器可见路径中。
...
当构建目标是 `target=bin` 时,`feng build` 应在完成依赖图展平后,收集依赖树中 `.fb` 包提供的当前平台动态库（如 `.so` / `.dylib` / `.dll`）,并释放到可执行文件目录,以满足运行时查找优先命中。
...
feng src/*.ff --pkg a.fb --pkg b.fb
    ...
    ├─ 处理 use，定位 .ft，类型检查
    ├─ 收集 @cdecl 注解，生成链接参数
    ├─ 从 .fb 读取当前平台实际存在的 lib/clib，生成链接参数
    └─ 调用 C 链接器，产出最终二进制
```
