...
- **编译器（`feng` 直接模式）**: 接受源文件列表和包路径列表,负责类型检查、代码生成和链接参数收集; 对 `--pkg` 指定的 `.fb`,编译器只直接读取其中的公开 `.ft` 与正式库文件; 编译器不读取任何 `feng.fm`。
...
编译器不接受包名、版本号或搜索路径,不读取任何 `feng.fm`。对 `--pkg` 指定的 `.fb`,编译器只直接读取其中的公开 `.ft` 与对应平台正式库文件（如 `.a`、`.lib`、`.so`、`.dylib`、`.dll`）; `.o` / `.obj` 不属于 `.fb` 的稳定分发接口。当前版本不支持交叉编译,始终以当前平台为目标平台。
...
另外,编译器会自动补入自身随附的基础 runtime 静态库:

- runtime 库: `build/lib/libfeng_runtime.a`,承载托管对象、数组、异常等运行时机制。

编译器私有的 runtime contract helper 若存在,也统一由这套 runtime 库提供,不再单独产出 intrinsic 静态库。
