...
- **编译器（`feng` 直接模式）**: 接受源文件列表和包路径列表,负责类型检查、代码生成和链接参数收集; 对 `--pkg` 指定的 `.fb`,编译器只直接读取其中的 `.fi` 与正式库文件; 编译器不读取任何 `feng.fm`。
...
编译器不接受包名、版本号或搜索路径,不读取任何 `feng.fm`。对 `--pkg` 指定的 `.fb`,编译器只直接读取其中的 `.fi` 与对应平台正式库文件（如 `.a`、`.lib`、`.so`、`.dylib`、`.dll`）; `.o` / `.obj` 不属于 `.fb` 的稳定分发接口。当前版本不支持交叉编译,始终以当前平台为目标平台。
...
编译器根据 `.fi` 中的声明关键字与 `@fixed` / `extern` 元信息,并结合 `.fb` 内实际存在的目录与文件,自动确定当前平台可用的链接目标:
...
构建工具传入的是**已确定并展平的 `.fb` 路径列表**,不传包名、版本或搜索路径。编译器只认路径,并直接从这些 `.fb` 中读取所需 `.fi` 与正式库文件。
