...
只要依赖图中存在非本地依赖,安装流程就必须先判定是否存在可用 registry。若没有可用 registry,命令必须在安装对应 `<name>@<version>` 时显式失败,并同时给出“安装哪个依赖失败”与“失败原因是未配置 registry”这两层信息。
...
- 只要存在远程依赖,必须先确认当前项目或全局配置提供了可用 registry; 若没有,则在安装具体 `<name>@<version>` 时明确报错
- 若存在可用 registry,则必须按 `${registry}/packages/${name}-${version}.fb` 拉取对应分发包; 下载失败时必须明确指出安装的依赖、所使用的 registry/URL 以及底层失败原因
- 远程包下载完成后必须立即校验其合法性; 至少要验证该文件可作为 `.fb` 打开、内置 `feng.fm` 可解析为合法 bundle manifest,且包名与版本精确匹配当前请求; 校验失败时必须明确指出安装的依赖及非法原因
...
- 缺少 registry 但遇到远程依赖
- 缺少 registry 时,诊断中必须同时包含失败的依赖坐标 `<name>@<version>` 与“未配置可用 registry”原因
...
- 远程包下载后不合法时,诊断中必须同时包含失败的依赖坐标 `<name>@<version>` 与具体非法原因
- 远程依赖安装诊断应以依赖坐标和失败原因作为主信息,不得把 `~/.feng/cache` 等缓存目录路径作为用户可见诊断前缀
