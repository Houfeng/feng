...
`type` 把数据和行为组织为具名类型。对象类型是托管引用类型；具名元组是值类型。详细规则见[类型规范](../../specifications/feng-type.md)。
...
没有显式构造函数时，可使用默认无参构造和对象字面量。对象字面量也可以在构造后覆盖尚可绑定或可写的成员。
...
圆括号形式声明具名元组；`@value` 可用于声明值语义对象类型。二者的复制与生命周期规则不同于普通对象类型，选择前应阅读[元组规范](../../specifications/feng-tuple.md)和[类型规范](../../specifications/feng-type.md)。
