...
- 泛型间距遵循[Feng 代码风格建议](../feng-style.md)第 5.2 节“泛型角括号紧贴建议”，覆盖带初始化器的字段和变量类型标注。
- 泛型实参中的指针及其组合后缀遵循[Feng 代码风格建议](../feng-style.md)第 5.1 节“类型后缀紧贴建议”。
- 变长参数 `T...` 与预打包转发 `...expr` 的紧邻关系；`...expr` 作为非首实参时保留逗号后的一个空格。
