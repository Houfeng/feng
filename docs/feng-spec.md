...
- `spec` 只约束可见形状（字段名、字段类型、行为签名与返回类型）,不约束具体内存布局、对象物理结构或 ABI 值布局。
- `spec` 中的成员与行为签名默认公开,且不允许显式添加 `pu` 或 `pr`。
- 对象形状中的行为签名使用 `fn` 关键字,在 `spec` 中可不写函数体。
- `spec` 行为签名的参数不可使用 `let` 或 `var` 修饰符; 参数可变性属于实现侧内部约束,不属于 `spec` 契约形状的一部分。
...
- [必须] 判断 `type` 是否满足 `spec` 时,字段匹配采用“名称 + 类型完全一致”规则。
...
- [禁止] `spec` 行为签名的参数列表中使用 `let` 或 `var` 修饰符。
...
- 每个 `spec` 都有对应默认 witness,且默认 witness 必须满足该 `spec` 声明的全部成员与签名要求。
- 字段成员按各自类型的默认零值初始化。
