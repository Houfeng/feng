...
- 通过 `import` 引入到当前文件作用域的同名 `type`、同名 `enum` 或同名顶层 `func`,不会与其他来源的声明共同组成新的重载集合; 只要在当前文件可见作用域内形成重名,即编译期报错。
- 通过 `import` 引入的公开模块级 `let` / `var` 不参与重载; 若与当前可见作用域中的其他值名形成冲突,同样编译期报错。
- 当前文件内新声明的 `type`、`enum`、顶层 `func` 或模块级 `let` / `var`,若与已通过 `import` 引入的可见名称重名,同样编译期报错。
