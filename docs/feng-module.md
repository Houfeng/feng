...
- `import` 会把目标模块或包中的公开 `type`、公开 `enum`、公开 `spec`、公开模块级顶层 `func`、公开模块级 `let` / `var` 引入当前文件的可见作用域，同时引入目标模块中以 `open fit` 导出的契约关系；引入后，该契约关系在当前文件作用域内生效。
- 支持导入源码模块、feng 自有二进制包、C ABI 兼容包。
- 支持使用 `import 模块名 as 别名` 为导入目标声明文件内别名。
- 通过 `import` 引入到当前文件作用域的 `type`、`enum`、`spec`、顶层 `func` 或模块级 `let` / `var`,不会与其他来源的声明共同组成新的重载集合。重名冲突按以下规则处理,均为编译期错误。

惰性检查（涉及 `import` 的冲突）:

以下三类冲突均在代码使用裸名引用时检测,未使用时不报错; 使用时报二义性冲突错误。该规则对 `type`、`enum`、`func`、`spec`、`let` / `var` 统一适用。

- 来自不同 `import` 模块的同名符号。
- `import` 引入的符号与当前 `module` 其他文件中定义的同名符号。
- `import` 引入的符号与当前文件内本地声明的同名符号。
...
Feng 的符号决议机制遵循以下核心原则,指导编译器在名称查找与冲突检测时的行为。以下规则对 `type`、`enum`、`func`、`spec`、`let` / `var` 统一适用。

同名符号冲突规则:

- **import vs import**: 惰性报错,在使用处报二义性冲突错误。
- **import vs 本模块其他文件**: 惰性报错,在使用处报二义性冲突错误。
- **import vs 本文件**: 惰性报错,在使用处报二义性冲突错误。
- **非 func,同模块（无论是否同文件）**: 急切报错,在定义处报重复定义错误。
- **func,同模块（无论是否同文件）**: 允许重载,按重载规则检查,如果重载冲突则急切报错。
- **别名**: 别名与本文件任何符号或其他别名冲突,急切报错。

### 7.1 惰性碰撞（Lazy Collision）

当当前文件作用域中存在来自多个来源的同名符号时,编译器不在 `import` 或声明发生时立即报错,而应在代码**真正引用该符号（标识符决议）时**才检查是否存在二义性。

规则说明:

- 若代码从未引用某个存在潜在同名冲突的名称,即使当前作用域中同时存在该名称的多个候选,也不产生编译错误。
- 仅当代码中使用了一个在当前作用域中存在多个同优先级来源的裸名时,才报二义性冲突错误。
- 本地声明与 `import` 引入的名称处于同一优先级,本地声明不会自动隐藏导入名称; 使用裸名时同样触发二义性检查。
- **例外（急切检查）**: `import` 别名（`as` 声明的名称）与当前文件内的任何符号或其他别名重名时,在声明时即报重复定义错误。别名是显式声明,其冲突是确定的,不属于潜在二义性。
...
以下场景覆盖多文件、同模块、外部导入联动时的关键边界。以下规则对 `type`、`enum`、`func`、`spec`、`let` / `var` 统一适用。
...
**场景 4（惰性碰撞 — import vs 本文件本地声明）**: 文件 A 中 `import app.ext1`,同时在文件 A 本地定义了 `type Foo`。

- 若 A 中未使用裸名 `Foo`,编译通过,无错误。
- 若 A 中使用裸名 `Foo`,报二义性冲突: `Foo` 同时来自当前文件本地定义和 `import app.ext1` 的导入。
- 可通过完整模块路径（如 `app.core.Foo` 或 `app.ext1.Foo`）或 `import ... as` 别名消除歧义。

**场景 5（急切检查 — 别名冲突）**: 文件 A 中定义了 `type Foo`,同时使用 `import app.ext1 as Foo`。

- 编译期在 `import ... as Foo` 声明处直接报重复定义错误,无需等到代码引用 `Foo`。
