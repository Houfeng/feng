...
- `import` 会把目标模块或包中的公开 `type`、公开 `enum`、公开模块级顶层 `func`、公开模块级 `let` / `var` 引入当前模块的可见作用域，同时引入目标模块中以 `open fit` 导出的契约关系；引入后，该契约关系在当前模块作用域内生效。
- 支持导入源码模块、feng 自有二进制包、C ABI 兼容包。
- 支持使用 `import 模块名 as 别名` 为导入目标声明文件内别名。
- 通过 `import` 引入到当前模块作用域的同名 `type`、同名 `enum` 或同名顶层 `func`,不会与其他来源的声明共同组成新的重载集合; 只要在当前可见作用域内形成重名,即编译期报错。
- 通过 `import` 引入的公开模块级 `let` / `var` 不参与重载; 若与当前可见作用域中的其他值名形成冲突,同样编译期报错。
- 当前模块内新声明的 `type`、`enum`、顶层 `func` 或模块级 `let` / `var`,若与已通过 `import` 引入的可见名称重名,同样编译期报错。
...
- 若未使用 `as`,则目标模块的公开 `type`、公开 `enum`、顶层 `func` 与模块级 `let` / `var` 以短名直接进入当前模块作用域。
...
- 同一 `module` 分布在多个文件中时,语言规范不对这些文件中的模块级 `let` / `var` 初始化先后顺序作出保证。
- 编译器内部可以自行选择稳定实现策略,例如按文件的标准化相对路径字典序处理,但该顺序不属于对外承诺的语言语义,用户代码不得依赖。
- 一个 `module` 若 `import` 了其他 `module`,则总是先初始化这些 `import` 目标,再初始化当前 `module` 自身。
- 同一文件内存在多个 `import` 声明时,按声明书写顺序依次初始化对应模块。
- 一个模块即使在多个位置被 `import`,也只在首次需要时初始化一次; 后续 `import` 或访问该模块成员都不会重复触发初始化。
- 在初始化过程中,若读取到某个尚未完成初始化表达式的模块级变量,则读取结果为该变量当前存储中的值; 在其初始化表达式执行完成前,该值初始情况下为类型默认值。
- `import` 循环不会单独导致编译期报错; 若循环中的某一侧在对方完成初始化前读取其模块级变量,则读取到的也是该变量当时的当前值,初始情况下为类型默认值。

重点提示:

- 同一 `module` 中,若模块级 `let` / `var` 分布在不同文件,则这些初始化的相对顺序不受语言规范保障。
- 一个 `module` 若 `import` 了其他 `module`,则总是先初始化这些 `import` 目标,再初始化当前 `module` 自身; 多个 `import` 按声明顺序初始化。
- 一个模块即使在多个位置被 `import`,也只会执行一次模块级初始化,不会重复初始化。
- 同一 `module` 跨文件依赖与 `import` 循环采用同一套语义: 若读取尚未完成初始化的模块级变量,则得到该变量当前值,初始情况下为类型默认值。
