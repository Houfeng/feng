...
`@iterable` 标注在 `type` 的某个成员方法上，表示该方法是该类型的"产生游标"入口。

约束：

- 每个 `type` 至多声明一个 `@iterable` 方法；超过一个时编译期报错。
- 被标注的方法必须无参数（除隐式 `self`）。
- 被标注的方法的返回值类型上必须存在恰好一个 `@iterator` 方法；若不满足，编译期报错。
- `@iterable` 与 `@iterator` 不得同时出现在同一个 `type` 上；违反时编译期报错。

### 2.2 `@iterator`

`@iterator` 标注在 `type` 的某个成员方法上，表示该方法是该游标类型的"推进迭代"入口。

约束：

- 每个 `type` 至多声明一个 `@iterator` 方法；超过一个时编译期报错。
- 被标注的方法必须无参数（除隐式 `self`）。
...
| 约束 | 说明 |
| --- | --- |
| `@iterable` 至多一个 | 每个 `type` 至多一个 `@iterable` 方法，超出编译期报错 |
| `@iterator` 至多一个 | 每个 `type` 至多一个 `@iterator` 方法，超出编译期报错 |
| 不可同时拥有 | 同一 `type` 不得同时有 `@iterable` 和 `@iterator`，违反编译期报错 |
