...
FC 支持 FC 自有二进制包 `.fp` 与 C ABI 兼容二进制包 `.fcp` 两种标准化包分发模式,用于满足 FC 内部复用与跨语言复用两类场景。

本规范仅保留包系统概要说明,详细的包格式、清单文件、接口文件、编译与使用流程、约束规则和导入语法已拆分到独立文档: [FC语言包分发与二进制复用规范](./feng-package.md)。

包系统要点:

- 支持 `.fp` 与 `.fcp` 两种标准化包格式。
- `.fp` 适用于 FC 内部公有成员的二进制复用,不面向 C 语言调用。
...
- 全自动 GC,不管理 C 兼容类型内存
- 支持 `.fp`(FC 自有包)和 `.fcp`(C ABI 包)双分发模式
- 二进制包不含源码,支持闭源复用与编译加速
- `.fp` 包仅 FC 间复用,`.fcp` 包支持跨语言复用
