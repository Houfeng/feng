...
- 孤儿适配（`A` 与 `B` 实现源码均不在当前包中）在当前包内正常建立契约关系并生效；其有效性不超出当前包，`pu fit` 形式的孤儿适配由编译器移除导出并输出肯定式提示（非错误、非警告）。纯自扩展 `pu fit A { ... }` 不适用该导出降级规则。
...
- [必须] 孤儿适配导出限制仅适用于带 `spec` 右侧的契约适配；纯自扩展 `pu fit A { ... }` 若声明本身合法，则可按普通 `pu fit` 导出其扩展成员能力。
...
- [说明] 孤儿适配规则：`fit A: B` 或 `pu fit A: B` 中 `A` 与 `B` 实现源码均不在当前包中时，该声明在当前包内正常有效；`pu fit` 形式的孤儿适配不得导出，编译器移除其导出并输出肯定式提示（非错误、非警告），明确告知有风险的公开契约导出已被移除。纯自扩展 `fit A { ... }` / `pu fit A { ... }` 不属于该规则。
...
- 编译器必须识别孤儿适配（`A` 与 `B` 实现源码均不在当前包中）；普通 `fit` 形式的孤儿适配在当前包内正常有效，编译器无需额外处理；`pu fit` 形式的孤儿适配，编译器必须移除其导出并输出肯定式提示消息（非错误、非警告），明确告知用户有风险的公开契约导出已被移除。纯自扩展 `pu fit A { ... }` 不适用该导出降级规则。
