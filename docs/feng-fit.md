...
- 孤儿适配: `fit A: B` 中 `A` 与 `B` 均不属于当前包的满足声明；编译器在编译期移除其元信息，不影响其他契约关系或对两类型本身的使用。
...
- 孤儿适配（`A` 与 `B` 均不属于当前包）在编译期由编译器移除元信息，不影响其他契约关系或对两类型本身的使用。
...
- [说明] 孤儿适配规则：若 `fit A: B` 或 `pu fit A: B` 中 `A` 与 `B` 均不属于当前包，编译器在编译期移除该满足声明的元信息，不影响其他契约关系及对两类型本身的使用；`pu fit` 形式的孤儿适配，编译器额外输出肯定式提示（非错误、非警告），明确告知有风险的公开契约导出已被移除。
...
- 编译器必须识别孤儿适配（`A` 与 `B` 均不属于当前包），在编译期移除其元信息；对 `pu fit` 形式的孤儿适配，编译器还须输出肯定式提示消息（非错误、非警告），明确告知用户有风险的公开契约导出已被移除。
