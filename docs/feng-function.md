...
- 编译器必须检查 Lambda 值是否位于显式 callable-form `spec` 目标类型上下文中; 对显式 callable-form `spec` 返回类型下的 `return` Lambda,必须检查 Lambda 参数与返回结果是否匹配该返回类型; 对无显式 callable-form `spec` 返回类型的 `return` Lambda,必须报错并阻止通过。
