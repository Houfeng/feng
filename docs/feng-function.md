...
- 模块级顶层 `fn`、成员方法与构造函数都支持重载; 重载只按“名称 + 参数个数 + 参数类型”区分,返回值不参与重载。
- 若同一重载集合中的两个候选在参数类型上存在可重叠匹配,包括因当前可见的显式契约关系而产生的重叠,则该重载集合非法,编译期报错。
...
- 模块级顶层函数、成员方法与构造函数的重载决议只在各自所属声明作用域内部进行,返回值不参与重载。
- 若两个候选仅返回值不同而参数列表完全相同,则视为签名冲突,编译期报错。
- 若两个候选的参数类型在当前可见的显式契约关系下可能同时匹配同一实参类型,则同样视为签名冲突,编译期报错。
- 通过 `use` 导入到当前模块作用域的同名函数,不会与本模块内声明的顶层 `fn` 或其他导入来源共同组成新的重载集合。
...
- 当模块级顶层 `fn` 或成员方法存在重载时,若它们以函数值形式出现,则必须存在足以唯一确定目标函数类型的上下文; 若当前上下文无法唯一选定某个重载,则编译期报错。
...
上例中,`u.say` 与普通 `Lambda` 或顶层函数不同,它在取值时已经把 `self` 绑定到 `u` 所引用的实例; 传入 `test` 时,由于目标类型 `M0` 能唯一确定无参重载,因此 `m()` 不会重新决定 `self`。`let s_ok: M0 = u.say;` 同样合法,因为显式声明的目标函数类型足以唯一选定无参重载。而 `let s = u.say;` 缺少足以唯一选定重载的目标函数类型,因此必须在编译期报错。
