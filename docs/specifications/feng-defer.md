...
- `defer` 块的作用域与可见性规则与普通块作用域一致,差异仅在于执行时机延后到块退出时。
...
- `defer` 块内调用产生的异常必须在该块退出前处理,边界检查见 §6。
...
- `defer` 块自身不得向外传播异常。编译器消费[异常元信息](./feng-exception.md#6-编译期异常摘要),只有证明块的逃逸异常集合为空才允许通过;已知异常、`unknown` 和未消除的开放泛型依赖均不能视为空集合。
- 检查涵盖块内表达式求值、直接和间接调用、隐式调用以及内部 `catch` 的执行。内部完整捕获且处理代码不再逃逸时可以通过;外层块、所属函数或调用者的 `catch` 不能替代 `defer` 自身的处理。
- 泛型函数、方法和 Lambda 的 `defer` 同样需要在其声明环境中证明安全,不能仅凭某个调用点恰好传入不抛异常的参数而放行开放的清理边界。普通分支条件不用于消除可能的异常。
- §4 的直接 `throw` 语句禁令继续生效,不因其处于内部 `try/catch` 中而放宽。被调用函数可以抛出异常,但其异常必须在 `defer` 边界内完整处理。
- 无法证明无异常逃逸时报告编译期错误 `AE1507`,定位到 `defer` 关键字并列出可能的异常类型;可定位的块内相关调用通过关联位置报告。CLI 与 LSP 使用相同检查结果,不受 Hover 展示设置影响。
