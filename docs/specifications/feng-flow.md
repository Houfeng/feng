...
- 循环变量仅在循环体内可见；其绑定实例化模型等同于在每轮进入循环体前执行一次相应的 `let` 或
  `var` 局部绑定声明：每轮均产生独立的新绑定，并以本轮元素值初始化该绑定。
- Lambda 捕获循环变量时只捕获当前轮的绑定；同轮多个 Lambda 共享该轮绑定，不同轮的 Lambda 不
  共享绑定。先形成 Lambda、再修改同轮 `var` 循环变量时，该 Lambda 读取修改后的值；`let` 循环
  变量仍不可重新赋值。捕获方式统一遵循 [Feng 语言函数规范](./feng-function.md)的引用捕获规则。
- 循环序列支持数组 `T[]`、`T[!]`，以及满足
