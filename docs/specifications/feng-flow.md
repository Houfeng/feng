...
1. 当前表达式上下文提供目标类型时，每个结果分支分别向该目标类型贴合并接受类型检查；分支之间不要求先推导为同一静态类型。所有分支均贴合成功后，整个表达式的结果类型为该目标类型。
...
这里的“可贴合数值字面量”包括纯数值字面量及纯字面量数值常量表达式，具体定义与贴合能力以数值类型规范为准。上下文目标类型包括显式类型绑定、函数或方法参数、函数返回值、成员或局部赋值目标，以及其他能够为当前表达式提供确定目标类型的上下文。上述规则不引入额外的隐式类型转换；任一正常分支无法贴合时，表达式类型检查失败。
