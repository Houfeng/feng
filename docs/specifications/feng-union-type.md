...
match v {
  ...
  n: Named {
    ...
    // 匹配其余未命中的情况；不建立绑定，v 仍为原 union 类型
  }
}
```
...
- `else` 匹配其余未命中的情况，不建立绑定，也不改变原绑定的类型；即使剩余 member 只有一个，原变量仍保持原 union 类型。需要使用 member 能力时，必须通过显式匹配建立新的收窄绑定。
...
赋值、初始化、传参、返回等进入站点确定 active member 时，编译器在**编译期语义分析阶段**通过成员图查找，确定从源类型到目标 union-form 的完整路径。路径信息传递给代码生成阶段，运行时仅执行已确定的 tag 设置与数据拷贝，**不存在动态匹配或递归**。

查找规则（2026-09-08 人工确认）：

1. **同类型绑定**：源类型就是同一完整目标 union 类型时，按普通同类型绑定处理，保留已有 active member，不重新选择路径。
2. **第一轮：精确匹配**。从目标 union 开始按广度优先查找，先直接成员，再逐层检查嵌套成员；同层按成员声明顺序从左到右，选择首个与源类型精确一致的完整路径。精确比较使用完整类型身份及泛型实参，不只比较名称。数值字面量使用目标贴合前的源类型；没有其他确定类型上下文时，使用内建类型规范规定的默认推导类型。
3. **第二轮：满足匹配**。只有第一轮在所有层级都没有命中，才重新从目标 union 开始，以相同的广度优先、同层从左到右顺序，选择首个符合既有转换或目标贴合规则的完整路径。例如，具体类型可按既有规则进入其满足的 object-form `spec`，数值字面量可按叶子数值类型贴合。
4. **失败与多候选**：两轮均无合法路径时，编译期报错。存在多条合法成员路径本身不再构成歧义；按对应轮次的首个命中选定。成员自身的名称解析、契约满足、转换资格及重载合法性仍按各自主规范检查，不因路径有顺序而放宽。

上述两轮必须作用于整个目标成员图，不能在某个子树精确匹配失败后立即做满足匹配，从而抢占其他子树的精确命中。深层精确匹配优于浅层满足匹配；同一轮内则浅层优先、同层左侧优先。合法无环共享子图中的不同路径分别参与排序，不按叶子类型全局去重；各 union 的直接成员仍按 §3.8 归一化。

初始化、重新赋值、实参、返回、字段／数组写入及 `if`／`match`／`try` 分支结果等相同目标类型的进入点必须复用同一选择规则，不能因入口不同改变 active member。选定的叶子类型与完整成员索引路径一起交给 Codegen，不只保留叶子类型后再次猜测路径。显式先得到某个成员类型的值，仍可用于主动指定进入路径。
...
spec Z: X | Y;
let i: int = 42;
let z: Z = i;                    // path = [X, int]
let via_y: Z = (Y)i;             // 显式指定 Y，path = [Y]
```
...
- 以上首成员展开只发生在无初始化器绑定或对象构造第一阶段等位置实际请求 union-form 默认零值时。若绑定具有
  显式初始化器，或对象字段具有声明初始化器，则直接按该初始化值确定 active member，不请求
  union-form 默认零值；即使首成员沿类型关系递归，也不得因此拒绝该初始化或对象构造。
...
- 共享泛型体必须提供与普通 union 值相同的收窄能力，不限于直接 member 的单层匹配。若绑定得到的 member 仍是 union-form，允许对该绑定继续 `match`，逐级收窄；也允许用 §3.13 的 `->` 链一次表达多级收窄。`match` 语句、块式表达式及 infix 形式均按各自既有规则支持，不因采用共享泛型体而限制合法路径。
- 逐级收窄的绑定具有该层 member 类型，链式收窄的绑定具有路径末端 member 类型；它们按 §3.6 的普通绑定规则继续使用。原参数仍保持 `V` 的静态类型与既有值语义，不能把获得 union 约束视角等同于将原参数改为约束 union 类型；具体承载与共享发码方案不改变这些语义要求。
...
- 对已确定的实际类型 `A`，是否满足 union-form 约束 `U`，与一个静态类型为 `A` 的值能否按普通联合绑定规则进入 `U` 一致：复用 §3.8.1 的编译期类型匹配及完整进入路径选择。同一完整闭合 `U` 自身按普通同类型绑定合法；精确 member、嵌套 member、间接叶子及经既有隐式 `spec` 转换进入 member 的类型，均按同一检查判定，不另设泛型专用的准入优先级。
- 上述复用的是类型级资格证明，不是把实参值自动转换到约束 union 的值流站点。`A` 仍是实际类型实参，参数、返回和默认零值不因此变成 `U`；也不能用某个字面量的目标类型贴合资格代替整个 `A` 类型的约束证明。不满足的类型必须在 Semantic 阶段拒绝，不能由当前 active member 或运行时试探决定准入。
- 开放泛参继续转传到 union-form 约束时，必须在 Semantic 阶段依据来源约束证明其允许的所有实际类型均满足目标约束；来源约束类型自身若可作为实际类型实参，也包括在证明范围内。复用上述联合绑定的类型关系及既有约束蕴含检查，不仅检查已知的某一个调用实例，也不能仅因两组叶子看起来相同就判为可转传。无约束、来源较弱或无法证明满足的转传应在当前泛型声明处报错，不延后到运行时检查。
- union-form 作为泛型类型实参时的类型保持、值传递及开销要求，见 [泛型主规范](./feng-generics-draft.md#4-语义)；约束准入不引入任意 union 之间的结构转换或泛型实例的 variance。

### 3.13 多级 match 语法

`->` 链式模式表达沿嵌套 union 层级进行的多级匹配，与逐级绑定后继续 `match` 使用相同的成员路径规则；不展平联合类型，也不引入新的联合值表示。普通 union 值与 §3.12 的 union 约束泛型值均适用。

**多级模式层级规则**：每一级必须是前一级类型的成员。

- 第一级：必须是 match 目标的 union 视角的直接成员；对受约束泛参，该视角由其 union-form 约束提供。
...
逐级写法通过 `expression` 绑定取得内层 union 视角，再继续匹配；无绑定分支不会使原始 `body` 自动收窄。有绑定链式标签 `[let|var] name: A -> B -> C` 则把 `name` 绑定为末端的 `C` 类型。

**编译器处理**：在编译期验证完整成员路径和最终绑定类型；各层只有在父层匹配后才继续判别，不能读取未选中 member 的 payload。路径上的成员关系不留到运行时搜索或检查。穷尽性检查按层级验证。
...
- union-form 还需要保证：
  ...
  - 首成员默认零值资格只在实际请求 union-form 默认零值时检查；显式绑定初始化器或字段声明
    初始化器直接提供 union 值时，不展开首成员默认零值；
...
- `else` 的剩余集合仅描述匹配范围，不改变原绑定类型，也不建立新绑定；具体规则见 §3.6。
...
### 7.5 重叠 member 的路径选择与转换成本

当前讨论已确认：

- 由于契约关系可能在后续声明中补全，且包内孤儿契约也会影响当前可见语义，member 的转换资格应在当前可见契约闭包固定后，于具体进入点检查。
- active member 唯一按 §3.8.1 选择；多个可接纳的 object-form `spec` member 不再触发旧的成员路径歧义诊断。若需其他路径，开发者仍可先显式转换为所需成员类型。
...
上例中，若 `user` 同时满足 `Named` 与 `Identified`，且没有精确 member，则 `let x: Display = user;` 按 §3.8.1 选择 `Named`；显式转换为 `Identified` 可选择另一成员。
