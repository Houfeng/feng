...
- 对已确定的实际类型 `A`，是否满足 union-form 约束 `U`，与一个静态类型为 `A` 的值能否按普通联合绑定规则进入 `U` 一致：复用 §3.8.1 的编译期类型匹配、完整进入路径和歧义检查。同一完整闭合 `U` 自身按普通同类型绑定合法；精确 member、嵌套 member、间接叶子及经既有隐式 `spec` 转换进入 member 的类型，均按同一检查判定，不另设泛型专用的准入优先级。
- 上述复用的是类型级资格证明，不是把实参值自动转换到约束 union 的值流站点。`A` 仍是实际类型实参，参数、返回和默认零值不因此变成 `U`；也不能用某个字面量的目标类型贴合资格代替整个 `A` 类型的约束证明。不满足或存在进入歧义的类型必须在 Semantic 阶段拒绝，不能由当前 active member 或运行时试探决定准入。
