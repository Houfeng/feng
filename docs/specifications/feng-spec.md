...
- `spec S` 的 `seal` 成员只允许在以下实现域中通过能够暴露该成员的 spec
  ...
  上下文不得访问。
- 从父 `spec` 叠加得到的 `spec seal` 成员保留原声明可见性，访问域按成员原声明 spec
...
- callable-form `spec` 的隐式匹配采用两段规则: 未绑定到 `spec` 的非泛型顶层函数、非泛型方法值、已通过显式泛型 target 完成闭合的函数或方法以及 lambda，进入 callable-form `spec` 位置时按“参数个数 + 参数类型 + 参数顺序 + 变长参数形态 + 返回值类型完全一致”做结构匹配；来源函数或方法自身声明泛参时，必须先显式提供完整类型实参，目标 `spec` 不参与推导来源泛参。一旦值的静态类型已经是某个 callable-form `spec`,后续赋值、参数传递与返回匹配只允许同一 callable-form `spec` 声明。
...
- 不同 callable-form `spec` 即使签名完全一致也不得隐式互相匹配; 仅当两个 callable-form `spec` 在实例化后的参数类型、参数顺序、变长参数形态与返回类型完全一致时,才允许显式转换。
...
- [必须] object-form `spec` 的父 `spec` 关系表示父契约成员叠加，不表示 OOP 实现继承或方法覆盖；叠加后的完整实例方法面与完整静态方法面必须分别使用普通 `type` 方法的重载、重复签名、仅返回类型差异、变长参数冲突和可见契约关系重叠规则；重叠规则统一由 [Feng 语言函数规范](./feng-function.md) 定义。
- [必须] 父契约成员叠加前必须先按每条父类型引用完成 owner 泛参替换；同名不同参数且不重叠的方法全部保留为重载，同名同参数但返回类型不同的方法构成冲突。
- [必须] 当子 object-form `spec` 与父 `spec` 叠加出完整签名相同的方法 requirement 时，只合并为一个 requirement，并以子 `spec` 的声明为代表；“子 `spec` 优先”只表示该等价 requirement 的叠加去重，不表示 OOP 覆盖。同一个 `spec` 体内重复声明完整签名相同的方法仍是重复声明错误。
- [必须] object-form `spec` 的父字段按每条父类型引用完成 owner 泛参替换后叠加到
  子 `spec` 的完整字段面，并与子级直接声明字段统一检查。完整实例字段面或完整静态字段面
  中只要出现同名字段就构成冲突，即使类型与 `let` / `var` 完全一致也不得合并。实例字段
  与静态字段的 owner 不同，分属不同成员面，允许同名且必须保留为两个独立 requirement。
- [必须] object-form `spec` 的完整实例成员面中，实例字段与实例方法不得同名；完整静态
  成员面中，静态字段与静态方法不得同名。实例成员与静态成员的 owner 不同，因此实例
  字段与静态方法、静态字段与实例方法允许同名；实例字段与静态字段也允许同名，并分别
  保留为独立 requirement。父 `spec` 叠加到子 `spec` 的成员与子级直接成员执行同一检查。
...
- [必须] 未绑定到 callable-form `spec` 的非泛型顶层函数、非泛型方法值、已显式闭合的泛型函数或方法以及 lambda 在进入 callable-form `spec` 位置时,必须按“参数个数 + 参数类型 + 参数顺序 + 变长参数形态 + 返回值类型完全一致”进行结构匹配。
...
- [必须] 不同 callable-form `spec` 之间的显式转换仅在实例化后的完整签名（包括变长参数形态）完全一致时允许,且资格必须在编译期确定。
...
- 编译器必须分别构建 object-form `spec` 完成 owner 泛参替换后的完整实例方法面与完整静态方法面，并复用普通 `type` 方法的同一套重载合法性检查。
- 编译器必须在父契约成员叠加中只对完整签名相同的 requirement 按“子 `spec` 优先”去重；同名不同签名的方法必须保留为独立重载，不得按名称丢弃或替换。
- 编译器必须在语义阶段分别检查 object-form `spec` 完成父契约叠加后的完整实例字段面和
  完整静态字段面，拒绝同面同名字段；实例字段与静态字段同名时必须分别保留并使用独立的
  编译期槽身份，不得按裸名称合并。
