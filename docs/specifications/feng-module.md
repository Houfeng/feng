...
- `import` 别名（`as` 声明的名称）与当前文件声明的符号（本地 `type`、`enum`、`spec`、顶层 `func`、模块级 `let` / `var`、其他 `import` 别名）重名时，在声明时即报重复定义错误，即使未使用也报错。
- 别名与无别名 `import` 引入的公开名称、或同模块其他文件声明的名称重名时，未引用该名称不报错；实际引用时在使用点报二义性冲突，并指出冲突来源。`别名.成员` 中对首段别名的引用同样参与检查，不根据后续成员是否存在选择来源；别名不优先于这些同名来源。
