...
- 急切检查：`import` 别名（`as` 声明的名称）与当前文件声明的符号（本地 `type`、`enum`、`spec`、顶层 `func`、模块级 `let` / `var`、其他 `import` 别名）重名时，在声明检查阶段即报重复定义错误，即使未使用也报错。这里的本文件声明指文件级声明，不包含嵌套词法作用域中的局部绑定。
- 惰性检查一：别名与同模块其他文件声明的名称重名时，未引用该名称不报错，实际引用时在使用点报二义性冲突。
- 惰性检查二：别名与无别名 `import` 引入的公开名称重名时，未引用该名称不报错，实际引用时在使用点报二义性冲突。
- 两类惰性冲突均须指出冲突来源。`别名.成员` 中对首段别名的引用同样参与检查，类型引用和普通表达式中的限定名称都适用；不根据后续成员是否存在选择来源，别名不优先于这些同名来源。

别名只产生当前文件内的名称，不向所属 module 增加顶层声明。因此，别名与其他文件声明的冲突不属于 §2 的同模块声明重复；同模块顶层声明之间的非法重名仍在声明检查阶段报错，合法函数重载除外。无别名 import 引入名称与本文件或同模块其他文件顶层声明之间的冲突，继续遵循上文的惰性检查规则。
...
- 若初始化过程中出现循环访问（A 的初始化读取 B，B 的初始化又读取 A），语言不检测、打断或恢复该循环；每次绑定访问仍按普通规则触发对应的惰性初始化检查，行为等同于普通函数的无限递归调用，最终自然耗尽调用栈。用户必须避免实际执行的初始化循环。
...
- **非 func,同模块顶层声明（无论是否同文件，不包含文件级 import alias）**: 急切报错,在定义处报重复定义错误。
- **func,同模块（无论是否同文件）**: 允许重载,按重载规则检查,如果重载冲突则急切报错。
- **别名**: 与本文件声明的符号或其他别名冲突时急切报错；与无别名 import 引入的名称或同模块其他文件声明冲突时，在使用点惰性报错，具体规则见 §4。

### 7.1 惰性碰撞（Lazy Collision）

对 §4 列出的惰性冲突，编译器不在 `import` 或声明发生时立即报错，而应在代码**真正引用该符号（标识符决议）时**才检查是否存在二义性。

规则说明:

- 若代码从未引用某个存在潜在同名冲突的名称,即使当前作用域中同时存在该名称的多个候选,也不产生编译错误。
- 裸名引用以及限定名称中的别名引用，均按 §4 检查同名来源；不因后续成员访问而跳过冲突检查。
- 本文件或本模块其他文件的顶层声明与无别名 `import` 引入的名称处于同一优先级，顶层声明不会自动隐藏导入名称；使用裸名时同样触发二义性检查。局部词法绑定不属于这里的顶层声明。
- **例外（急切检查）**: `import` 别名与当前文件声明的符号或其他别名重名时，在声明时即报重复定义错误；涉及导入名称和同模块其他文件声明的别名冲突遵循 §4 的惰性检查规则。
...
- 编译期在声明检查阶段直接报重复定义错误，定位到本文件内相冲突的声明，无需等到代码引用 `Foo`。
