...
- 数组类型作为泛型容器，支持与具名泛型类型一致的静态方法访问。完整数组类型可作为静态成员访问目标，如 `i32[].method`、`E[].method`、`Box<E>[!][].method`；数组层级及逐层可写性均属于目标类型。直接调用、方法级类型实参、方法值及泛型上下文转发继续遵循通用 `fit` 与函数规则，类型目标本身不产生运行时数组实例。
...
- [必须] 数组静态方法从静态访问目标的元素类型绑定目标泛参；方法级泛参与目标泛参独立按声明身份及位置替换，不得依赖不同作用域的泛参是否同名。静态方法不接收 `self`。
- [必须] 数组静态方法值在具体目标处绑定完整目标类型与显式方法类型实参；在泛型函数中形成时，通过既有可实例化依赖传递这些信息，具体实例化后再绑定。方法值的可见性、重载、签名匹配与生命周期遵循通用函数规则。
...
- 编译器必须在静态成员访问中保留结构化的完整数组类型，包括具名泛型元素与逐层可写性，并复用既有静态成员语义选取和泛型调用协议。`[]` / `[!]` 类型后缀、`[index]` 下标与 `[:size]` 数组构造必须保持各自含义。
