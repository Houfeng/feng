...
符号表必须保留[异常规范 §6](./feng-exception.md#6-编译期异常摘要)的声明摘要及必要依赖。
发布表与 workspace 表使用相同表示，消费方只查询编译事实，不读取原函数体。
FT 保存结构化符号及相关编译信息、源码文档注释；不保存编译器诊断、提示文本或预格式化
的异常类型列表。诊断和 Hover 展示由消费方从结构化元信息生成。

新增必需节 `EXCEPTION_EFFECTS`（kind `0x000C`，flags `REQUIRED`，变长记录，entry_size 为 0）。
本次沿用现有 FT 版本并要求重编。缺失、重复或损坏的摘要节使产物无效；workspace 缓存按
既有失效路径回退到源码，发布包报告需要重新构建，不能把缺失摘要解释为不抛异常。

节内整数均为小端 u32；头部是根记录数、callable 模板数、摘要节点数及特征位。
当前唯一特征位 `STRUCTURED_ROOTS = 1` 必须设置，表示根记录保存求解后的节点引用；
其他位必须为零。旧文本根记录的特征位为零，须拒绝并要求重编；FT 版本号不变。
随后依次排列：

1. 根记录：symbol id、模板 id、已求解的声明异常根节点 id。模板用于调用点实参代入，
   已求解根用于直接查询该声明的可能异常，无需为 Hover 重新求解。
2. 模板记录：名称 STRS id、类型参数数、值参数数、捕获参数数、异常根节点 id、返回值
   依赖根节点 id、零保留字段，以及各类型参数显示名的 STRS id。
3. 节点记录：kind、x、y、z、可选 STRS id、子节点数、零保留字段，随后是子节点 id 数组。

节点仅表达类型身份、声明参数槽位、集合运算、捕获条件和调用／callable 值依赖。
具体类型名称使用经语义解析的完全限定身份；泛参由模板身份和槽位标识，类型实参保持
结构化子节点，不按显示名称替换。字符串共用 STRS。模板可相互引用以表达递归；节点的
子节点引用必须先于父节点，不能构造任意节点环。节点 0 是空集合，节点 1 为内部真值。

当前节点 kind 的编码如下。表中未使用的整数字段为 0，除注明的名称外 STRS id 为 0。
参数节点的 x 是模板 id、y 是该模板的槽位；其他引用均为节点 id，除函数／方法节点的
x 仍为模板 id。除条件决策节点外，z 为 0。

| kind | 含义 | 字段 |
| --- | --- | --- |
| 0 / 1 | 空集合／真值 | 无子节点，只能分别位于节点 0 / 1 |
| 2 | unknown | 无名称或原因字符串，子节点为已排除的类型 |
| 3 | 并集 | 至少两个同类子节点 |
| 4 | 名义类型 | 必需的完全限定名称字符串，子节点为类型实参 |
| 5 / 6 | 类型／值参数 | x=模板，y=槽位 |
| 7 | 具有静态类型的值 | x=类型 |
| 8 | 值来源及其静态类型 | x=值来源，y=类型；来源不透明时使用静态类型 |
| 9 | callable 值 | x=模板，子节点依次为类型实参、捕获值 |
| 10 | spec 成员值 | x=需求模板，y=subject 值，子节点为类型实参 |
| 11 / 12 | 调用异常／返回值依赖 | x=callable 值，子节点为值实参 |
| 13 | 抛出 | x=类型 |
| 14 / 15 | 排除／选择捕获类型 | x=异常集合，y=类型 |
| 16 | 条件贡献 | x=异常集合，非空时贡献 y 的异常集合或返回值 |
| 17 | 类型相等条件 | x、y=类型 |
| 18 | 条件决策 | x=类型相等条件，y=不相等分支，z=相等分支，分支为布尔条件 |
| 19 | 受条件约束的结果 | x=布尔条件，y=异常集合或值 |

类型、值、布尔条件和异常集合的引用类别均须校验。根记录的已求解结果只能由空集合、
unknown、抛出类型、其并集及条件组成，不能用未求解的调用依赖代替。仅保留选中声明根
依赖的结构，不导出其他调用点的查询缓存。

模板及节点 id 从 0 开始，仅在本节内有意义。所有索引、参数槽位、调用绑定长度、节点
种类及保留字段均须验证。函数、方法、构造函数、终结器的导出声明必须有且仅有一个根；
根模板的 owner 与 callable 类型参数总数、显式值参数数、receiver 捕获数必须与所指声明
一致；不得将有效模板 ID 错配给不同形状的声明或不支持摘要的符号类别。
必要的初始化和返回 callable 依赖也随对应声明保存。只导出所选根的依赖闭包，不保存
查询缓存、源码局部变量、原语句或通用控制流图；私有依赖不因此获得公开可见性。
...
公开签名必须先通过 [Feng 语言可见性规范](./feng-visibility.md) 的可见性一致性
检查；检查失败时不生成公开 `.ft`。
...
| `FT_SEC_EXCEPTION_EFFECTS` | `0x000C` | 必需（允许空节） | 全部 | 参数化异常摘要与必要依赖闭包 |
...
- `0x000A`、`0x000B` 分别用于下述视角形成、约束投影依赖节；`0x000D` 至 `0x000F` 预留。
