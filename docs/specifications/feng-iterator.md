...
循环变量的逐轮实例化、`let` / `var` 可变性及 Lambda 捕获身份统一遵循
[Feng 语言流程控制规范](./feng-flow.md)第 6.2 节；数组路径与迭代器协议路径不得产生不同的循环绑定
语义。
...
- **不引入额外堆分配**：未捕获循环变量时，`for/in` 展开本身不得额外分配堆内存；用户
  `@iterable` 实现自身分配的内存不在此限。源码 Lambda 实际捕获循环变量时，可按普通闭包规则仅
  为该捕获路径持久化对应绑定，不得把捕获开销扩散到未捕获路径。
- **循环变量无额外拷贝**：循环变量绑定应直接来自 `@iterator` 返回值的第二元素，不得引入中间临时
  拷贝；实际闭包捕获所需的绑定存储不属于迭代器协议中间临时值，但仍只允许在捕获路径生成。
