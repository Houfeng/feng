...
- 参数名称没有特殊含义；
- 声明方法的具体类型在声明头中名义声明满足第一个参数的 `spec`；如果方法声明在
  `fit Source` 中，这里的具体类型是 `Source`；
- 方法从来源展开到目标时，目标类型也在声明头中名义声明满足该 `spec`。

上述关系在生成 wrapper 前只检查声明头中的名义关系，不依赖当前 `@mixable` 方法或
其他生成成员预先证明完整满足。生成字段和 wrapper 进入普通成员表后，现有 `spec`
满足检查再验证声明类型和目标类型是否真正满足该契约；生成成员允许参与该最终判定。
...
- [必须] `@mixable` 方法的声明类型和展开目标都必须在声明头中名义声明满足第一个
  参数的 `spec`，完整满足关系由生成成员进入普通成员表后的现有检查验证。
...
- 编译器必须检查 `@mixable` 的适用位置、参数个数、首参数变长标记、首参数
  object-form `spec` 形态，以及声明类型和展开目标的名义 `spec` 声明关系。
