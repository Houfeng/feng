...
- Lambda 位于三段式 `for` 或 `for/in` 循环中时，具体捕获哪个动态绑定实例，统一由
  [Feng 语言流程控制规范](./feng-flow.md)第 6 节的循环绑定模型决定；本文的引用捕获规则只作用于
  已经确定的绑定实例，不重新定义循环逐轮实例化语义。
...
#### 4.1.1 非 `void` callable 的正常落尾

显式声明或经返回类型推导确定为非 `void`、且具有实现 body 的 callable，不允许存在能够正常到达
body 末尾的控制流路径。该规则适用于非 `extern` 顶层函数、具有 body 的普通实例方法、静态方法、
`fit` 方法和目标返回类型非 `void` 的块 Lambda；无 body 的声明不执行落尾检查，构造函数与终结器
继续使用类型规范定义的特殊返回规则。

本规则检查的是“body 末尾是否仍可正常到达”，不是要求每条执行路径都必须实际执行 `return`：

- 带值且类型合法的 `return expr;` 终止当前路径；空 `return;` 不满足非 `void` 返回契约；
- `throw` 产生异常控制流，不产生正常落尾。异常若离开当前 callable，该路径已经终止；异常若被当前
  callable 内包围它的 `try/catch` 捕获，则控制流转入匹配的 `catch`，由该 `catch` body 是否正常
  完成决定这条派生路径能否继续；
- 普通函数、方法或 Lambda 调用按其签名保留正常返回路径；编译器不根据被调用方当前 body 是否总是
  `throw` 进行跨 callable 终止推导。Feng 当前没有“永不返回”的 callable 签名契约，因此调用一个
  内部实现总是抛出的 `void` 辅助函数，不能代替当前非 `void` callable 自身明确的 `return`、`throw`
  或其他本节定义的终止控制流；
- 普通块按语句顺序合并控制流；只有前一语句仍可能正常完成时，后一语句才参与该路径；
- `if / else if / else` 只有存在最终 `else`，且每个分支都不产生正常落尾时，整体才不产生正常
  落尾；省略最终 `else` 时，条件均不成立的路径正常通过该语句；
- 块形式 `match` 只有存在 `else`，且所有普通分支与 `else` 都不产生正常落尾时，整体才不产生正常
  落尾；省略 `else` 时，未命中的路径正常通过该语句；
- `try/catch` 的主表达式正常完成时，控制流从整个结构之后继续；主表达式抛出的异常被本地
  `catch` 捕获后，每个可达 `catch` body 的控制流结果也参与合并。正常完成的主表达式或任一可达
  `catch` body 都会保留正常路径；未匹配的异常或在 `catch` 中重新抛出的异常继续作为异常控制流，
  不产生正常落尾；
- `while` 或三段式 `for` 的条件可能为 `false`，或者存在作用于当前循环的可达 `break` 时，循环后
  保持可达。`while` 条件为布尔字面量 `true`，或三段式 `for` 省略条件 / 条件为布尔字面量
  `true`，并且不存在作用于当前循环的可达 `break` 时，循环没有正常出口，循环后的路径不可达；
  嵌套循环中的 `break` 只影响其直接所属循环；
- `for/in` 可能零次迭代或在序列结束后退出，始终保留循环后的正常路径；循环体中出现 `return` 不
  能证明该循环必然返回；
- 能够证明没有正常出口的循环可以出现在非 `void` callable 中，即使循环体没有 `return`。例如
  `while true {}` 永不正常落尾，因此合法；本规则不要求永不结束的路径产生返回值。

省略返回类型时，编译器先按本节前述规则统一所有 `return` 的类型：完全没有有值 `return` 时仍推导
为 `void`；推导结果为非 `void` 时，再执行正常落尾检查。因此某个分支返回值、另一个分支正常落尾
的 callable 不会因省略返回类型而规避本规则。

正常落尾检查只属于编译期语义分析，不引入运行时返回检查、分支、调用、分配或 ARC。
...
- [必须] 显式声明或经返回类型推导确定为非 `void`、且具有实现 body 的非 `extern` 顶层函数、普通
  实例 / 静态 / `fit` 方法和块 Lambda，不得存在能够正常到达 body 末尾的路径；逃逸当前 callable
  的 `throw` 和可证明没有正常出口的循环终止相应路径，不要求实际产生返回值；本地捕获按 §4.1.1
  继续合并 `catch` 结果。
...
- [必须] 若同一重载集合中的两个候选在当前可见的显式契约关系下可能同时匹配同一具体实参类型,必须视为签名冲突。
- [必须] `type T` 与其满足的 object-form `spec S` 构成上述重叠；两个 object-form `spec` 参数仅在当前分析中存在至少一个同时满足二者的可见具体 `type` 时构成上述重叠。仅有子 `spec S1` 到父 `spec S2` 的关系、但没有可见具体 `type` 同时满足二者时，不构成声明期重载冲突，调用点继续按普通精确匹配优先级决议。已经构成重叠冲突的候选不得用调用点显式 cast 保留。
...
- 编译器必须以可扩展的控制流结果分析检查非 `void` callable 的正常落尾，不得仅扫描是否出现过
  `return`。分析至少区分正常通过、返回、抛出、作用于当前循环的 `break` 和 `continue`，并按
  §4.1.1 合并块、分支、`try/catch` 与循环；发现 body 末尾仍可正常到达时必须在 Semantic 阶段
  报错并阻止通过。
...
- 编译器必须检查顶层函数与成员方法的重载集合是否存在返回值冲突或重叠匹配冲突；重叠检查必须纳入具体 `type` 与其满足的 object-form `spec`，并在两个 object-form `spec` 参数之间检查当前分析中是否存在同时满足二者的可见具体 `type`。仅有子、父 `spec` 关系而没有这种具体 `type` 时不得判为重叠冲突；构造函数相关检查见 [Feng 语言类型规范](./feng-type.md)。
...
- 非 `void` callable 的正常落尾完整性已经在编译期确定；该语言规则不得在运行时增加缺失返回检查、
  分支、调用、分配或 ARC。永不正常退出的合法路径不需要构造返回值。
