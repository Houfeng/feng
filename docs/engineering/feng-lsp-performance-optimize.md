...
若当前文档从文件起始位置到请求位置仍与同一代 `last_successful_analysis` 的源码字节一致，则该请求位置仍可直接使用成功 AST 和语义结果；请求必须读取成功版本对应的源码，不能把当前文档缓冲区传给旧 AST。共享前缀以外的编辑不得阻断该位置的 Completion、Hover 等只读查询。请求位置位于共享前缀以外时，仍须通过当前 AST 或请求内 repair 重新确定作用域，禁止直接套用旧 AST offset。
...
成员访问位于括号、调用参数或下标等嵌套表达式中时，光标后的 `)`、`]` 只表示内层定界符已经闭合，不表示完整控制流头或语句已经结束。请求内 repair 必须保留已有闭合符，并在外层表达式的正确边界补充缺失语法：例如 `if (receiver.)` 应在右括号之后补充代码块，`consume(receiver.)` 在没有既有语句终止符时应在右括号之后补充分号；已有代码块或分号时不得重复补充。尾部插入位置必须依据通用的词法定界符和表达式边界确定，不得依赖换行缩进、被调用函数名或接收者名称。

上述嵌套表达式恢复规则同样适用于普通标识符前缀，不得仅覆盖成员访问。例如在 `consume(sel)`、`outer(consume(sel))`、`if (sel)` 或 `values[sel]` 中请求 Completion 时，即使外层语句尚未完成，也必须以当前文本的请求内 repair 安全确定所在声明和块作用域，再从最后一次成功发布的符号数据返回可见局部变量、`self`、当前模块声明和导入声明。已有右括号或右方括号必须保留，缺失的分号或控制流代码块必须补在外层表达式边界；不得直接以旧 AST offset 推断当前作用域，也不得在作用域无法确定时把所有缓存符号无条件返回。

Signature Help 的错误恢复遵守相同的表达式边界规则。当前调用尚未闭合时，请求内 repair 必须按词法定界符的真实嵌套顺序闭合当前调用及其未闭合的外层圆括号或方括号，并保留光标后已经存在的匹配闭合符；普通表达式补齐语句终止符，需要后续代码块的 `if`、`while`、`match` 或 `for ... in` 头则补齐可解析的代码块形态。例如，`user.commit(`、`consume(user.commit(`、`if user.commit(` 与 `if (user.commit(` 必须都能从当前文档作用域和已发布 `SymbolIndex` 返回同一被调用成员的完整重载集合。该恢复不得按被调用方法名、接收者名或文件路径增加特判，也不得同步触发整项目分析或替换最后一次成功缓存。

自由函数调用的 Signature Help 必须返回语言名称解析所选声明来源中的完整同名函数重载集合，包括普通、泛型和可变参数重载。本模块的可见声明优先；本模块没有匹配声明时，按当前程序既有的未别名导入顺序选择首个导出该函数的模块，并仅收集该模块中的公开同名函数。不得只返回名称查询命中的第一条声明，也不得把其他模块中碰巧同名的函数合并进当前重载集合。例如，导入 `std.io` 后的 `println(` 必须同时返回该模块导出的全部 `println` 重载。该查询只能读取当前 AST、最后一次成功发布数据和持久 `SymbolIndex`，不得新增请求内项目分析或磁盘读取。
