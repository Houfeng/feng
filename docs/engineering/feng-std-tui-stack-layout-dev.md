...
- 按 children 的 List 顺序垂直排列直接子组件；
- VStack 直接决定普通流 child 的最终 `frame.x/frame.y`；
- VStack 自主调度直接 children 的布局和绘制，ViewManager 仍只调度 root；
...
本阶段不实现 HStack、公共 measure 阶段、ArrangeSlot、全局布局重构、spacing、权重分配、
Flex、Grid、Dock、ScrollView、虚拟滚动或其他 Widget。

## 2 层级职责

VStack 只处理自己的直接 children，不跨级遍历或修改 descendants：

- VStack 调用每个直接 child 的 `doArrange(manager)`；
- child 自行计算尺寸和完成自身布局；
- VStack 根据 child 的最终尺寸写入该直接 child 的 `frame.x/frame.y`；
- 如果直接 child 本身是容器，由该 child 自行处理自己的直接 children；
- VStack 绘制时只按顺序调用直接 child 的 `doDraw(manager)`。
