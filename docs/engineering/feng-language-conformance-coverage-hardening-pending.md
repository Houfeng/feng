...
- [ ] MODULE05：按模块规范 §4 覆盖 alias 与本文件声明（含其他 alias）的急切冲突，分别验证未使用和已使用均被拒绝；覆盖 alias 与无别名 import 引入名称、同模块其他文件声明的惰性冲突，分别验证未使用通过、使用时报二义性，且诊断位于使用点并指出来源；包括 `alias.member` 访问，不按后续成员是否存在消歧。正向覆盖更换不冲突 alias 和完整模块路径访问；覆盖 import 排列及多文件输入顺序不改变结论。
- [ ] MODULE06：在未 import 的同模块其他文件中使用短名被拒绝；
- [ ] MODULE07：完整模块路径访问的最小合法邻界程序。

### 26.3 独立验收与交付 TODO

- [ ] 按 [ISSUE-G22-001](./feng-language-conformance-coverage-hardening-issues/g22.md#issue-g22-001别名与非本文件声明的名称冲突仍被急切拒绝) 修复冲突检查时机；既有测试仅迁移该记录明列的两条，其余需要修改时再次人工决策；新增用例覆盖未使用通过和使用点诊断，正向语言行为放入 fcts。
