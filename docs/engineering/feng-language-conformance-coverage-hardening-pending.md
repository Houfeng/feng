...
- [ ] COMPOSITE11：按联合主规范 §3.8.1 验证多个精确路径、多个可接纳 object-spec member、
...
- [ ] COMPOSITE37：依据联合主规范 §3.12／§3.13，以普通 union 参数函数为行为对照，在
...
- [ ] COMPOSITE38：按[共享体联合收窄设计 §7](./feng-generic-union-match-draft.md#7-元信息通道描述符的-reified_union_projections)
...
- [ ] COMPOSITE39：按设计 §5／§6／§8 验证投影表项的生成和消费。静态不可能路径不读取源值；
...
- [ ] COMPOSITE40：以至少三层共享调用验证投影依赖逐层可用。先使用已确定合法的相同联合
...
- [ ] COMPOSITE41：非共享体的普通 union 收窄建立独立不回归证据，覆盖普通 union 与已闭合的
  ...
  的语义、合法性边界或正常路径成本。
- [ ] COMPOSITE42：完全没有 union 使用的共享函数／类型／方法建立控制用例，覆盖无约束、
...
- [ ] 对 COMPOSITE01～COMPOSITE43 按各项语义的适用性核对正向／反向、声明／使用、同包／真实
  跨包、显式／推导、实际 `T` 为具体类型／spec、引用／值及边界组合；每个适用格记录复用或新增
  的直接证据。缺少运行断言、尚未复现或仍待决策的格子不得算完成，不以“全量通过”掩盖未覆盖规则。
- [ ] 建立具体诊断模板到源码可达入口的映射，优先核对 `AE0601`～`AE0607`、`AE0621`／`AE0622`
...
- [ ] 单列 COMPOSITE40～42 的多层转传、非共享 union 和无 union 共享体结果；既有测试通过
...
- 状态：实施中（2026-09-09）；COMPOSITE43 已独立完成，COMPOSITE27 的 051 仍未关闭，
  G24 整组尚未验收。
  必要构造调用按 §28.1.3.3 的限定范围验收，其他新增成本仍须另行决策。
  ISSUE-G24-006 本组暂缓消除优化，不阻塞；产品实现、语义、覆盖和成本均未验收
...
- 问题：[G24 问题记录](./feng-language-conformance-coverage-hardening-issues/g24.md)，当前包含
  ISSUE-G24-001～ISSUE-G24-062。008 两处调用及 009 两条旧发码断言均仅按明确批准迁移。
...
- 当前剩余：COMPOSITE27 最终核对独立复现交叉约束开放转传 `CE0293`，记录为 051。
  相关泛参描述符／witness 承载及运行时成本尚需人工 Review，未擅自增加临时描述符或 ABI 字段；
  G24 仍未完整交付，不以本次全量通过替代该缺口的后续修复和用例验收。
