...
已有的 spec 泛型转接层；其他增量成本仍须人工确认。最新已确认的描述符投影方案见 §28.1.2，
...
- 2026-09-08 人工确认：union 约束的共享泛型体按联合主规范 §3.12／§3.13 验收完整收窄能力；
  ...
  此确认不代替 COMPOSITE23 的准入边界决策，承载方案按以下后续讨论结论更新。
- 后续人工确认的[共享体联合收窄设计](./feng-generic-union-match-draft.md)采用
  ...
  字段及表项只在该设计文档定义；COMPOSITE38／39 覆盖承载契约，ABI／格式及成本门槛见 §28.1.3。
...
三个既有问题的修复目标、复合 spec 泛型能力及投影承载方向已确认；当前只完成文档收敛，产品实现
和正式测试尚未变更。未知准入关系、具体成本及 ABI／格式迁移按下节提交人工决定，不新增任意结构
转换或 variance。实施中问题按第 30 节先记录、再分析、再解决；既有测试迁移仍需逐项批准。

#### 28.1.3 开工评估与剩余确认项

2026-09-08 结论：投影承载和依赖排序设计已可作为实施依据，但 G24 尚不能标为“整体无未决项”。
ISSUE-G24-001～005 的已定缺口及明确的正反例可先安排；不因此缩减整组交付范围，也不把
ISSUE-G24-006 的获批延期重新设为阻塞。本次仅更新文档并评估，不开始产品实现。

##### 28.1.3.1 联合约束准入与开放转传

COMPOSITE23 已明确精确 member、直接内层 union、整个约束 union 自身合法，无关类型非法。
联合主规范 §3.8 明确的是值进入路径，§3.12 尚未明确把所有这种路径都用于泛型约束准入。
因此下面两类不能仅由“完整支持共享体收窄”自动判为允许；它们不是新发现的运行失败：
...
// 待确认：Item 不是精确 member，仅能按已声明名义关系转换到 Named。
func converted_member(value: Item): Item { return accept_named<Item>(value); }
```

建议允许具备唯一合法类型级进入路径的上述两类实际类型；开放转传须在 Semantic 证明来源约束
允许的实际类型均满足目标，不能把当前值的 tag 或任意 union 的结构相似性当作证明。
这是待人工确认的建议，不是本次偷偷新增的语言规则。确认后只在主规范定义，COMPOSITE23／27
引用并补齐合法、无路径、歧义和错误闭合实例的对应测试。

##### 28.1.3.2 私有描述符与 .ft 迁移

三种描述符增加字段的方向已确认，不再重复决定是否使用独立隐藏参数；但新旧编译器、runtime
和二进制包如何识别契约、需要重建哪些制品，仍须给出精确迁移方案并批准后再改 ABI／格式。
当前 .ft reader 检查 major、不拒绝更高 minor，不能只提高 minor 就声称旧工具链会安全拒绝。
该风险及证据归 ISSUE-G24-004 与设计 §7.5，不另起一个无关的格式改造专项。

##### 28.1.3.3 成本验收门槛

投影表的结构和编译期生成方式已明确，具体增量见设计 §8.2：描述符体积、表指针／表项读取、
静态路径判断，以及必要时的绑定构造入口调用。它们不能直接记为已满足早先“无新增成本”的要求。
原有临时描述符的初始化和闭包捕获也要核对，不能只检查新增匹配处。

建议以“不增加独立隐藏参数、不统一包装、不做运行时约束搜索；仅在所需路径消费静态投影并
构造必要绑定”为本方案的专项成本边界，接受范围仍待人工确认。确认前不靠降低覆盖范围、
隐藏间接调用或依赖同包内联绕过；后续实测和生成 C 证据按 COMPOSITE36／39 验收。
...
- [ ] COMPOSITE36：在同包及真实二进制跨包中，以本组变更前的同一合法程序及同类已支持路径
  ...
  ISSUE-G24-006 只延期消除优化，不豁免正确性和增量成本验收。新投影方案的专项成本必须
  按 §28.1.3.3 先明确接受范围，再逐项核对；不以一般 spec 的必要操作名义隐藏新增投影读取或
  构造入口间接调用。描述符结构和 `.ft` 的迁移按 §28.1.3.2 Review。
...
- [ ] COMPOSITE39：按设计 §5／§6／§8 验证投影表项的生成和消费。静态不可能路径不读取源值；
  ...
  普通绑定的值语义。无绑定或未命中分支不得调用 materialize；需要中间实体值时，只在绑定
  形成处直接初始化目标，不先请求默认零值，目标大小／清理按绑定结果类型处理。
...
- 状态：三个既有问题及复合 spec 泛型支持目标已批准，投影描述符承载与依赖归属已确认；
  COMPOSITE01～COMPOSITE42 待实施。整体开工尚有 §28.1.3 的准入、ABI／格式迁移和成本门槛
  待确认；ISSUE-G24-006 已批准本组暂缓消除优化，不再阻塞。语义、覆盖和成本均未验收
- 稳定码映射与新增用例：—
- 本组专项结果：—
- 本组沙箱外 `make test`：—
