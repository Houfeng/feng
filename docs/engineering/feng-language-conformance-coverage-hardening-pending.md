...
- [ ] ARRAY-D26：无显式目标时 `Type[:n]` 推导为 `T[]`，贴合显式 `T[]` / `T[!]` 时按目标决定当前
  ...
  长度改变类型结论；
- [ ] ARRAY-D27：补齐数组创建对元素默认零值资格的直接证据。有限默认零值元素正常创建；元素类型
  没有有限默认零值且创建会请求元素默认值时，必须在 Semantic 以 G18 的 `AE0332` 拒绝，不能留下
  NULL 元素或进入 Codegen 的 `CE`。常量零长度与动态可能为零时是否仍要求元素类型具备有限默认零值，
  必须先按 `ISSUE-G21-006` 取得人工决策，再固定正反向用例，不得在实现中自行加特判。
...
- [ ] 先实施并关闭 [G21 问题记录](./feng-language-conformance-coverage-hardening-issues/g21.md) 中的
  ...
  `ISSUE-G21-005`；`ISSUE-G21-006` 的元素默认零值资格边界必须先由人工决策，未决前不得实现
  ARRAY-D27；
...
- [ ] 运行完整 FCTS，确认 ARRAY-D04、ARRAY-D07～ARRAY-D08、ARRAY-D11～ARRAY-D19、ARRAY-D20、
  ARRAY-D23、ARRAY-D26、ARRAY-D32～ARRAY-D34 的合法行为均由主入口登记并真实执行；
...
- 状态：待实施；ARRAY-D27 尚待 `ISSUE-G21-006` 人工决策
- 稳定码映射与新增用例：—
- 本组专项结果：—
- 本组沙箱外 `make test`：—
- 性能与兼容性：—
