...
Review 后只保留分组范围、实施顺序、状态和本文链接；G01～G24 的用例 TODO 与验收记录在本文维护，
G25 在[独立实施文档](./feng-language-conformance-coverage-hardening-g25.md)中维护，本文第 29 节
只保留入口。详细问题记录只在第 30 节链接的对应问题文件中维护。
...
验证 object-form `spec` 的声明、完整成员要求、名义满足关系和 `fit` 实现选择，补齐非法程序的
编译期拒绝证据及合法适配的真实运行证据。规范依据为
[`spec` 规范](../specifications/feng-spec.md)、
[`fit` 规范](../specifications/feng-fit.md)、
[内建目标 fit 规范](../specifications/feng-fit-builtin-type.md)、
[tuple 规范的 fit 扩展](../specifications/feng-tuple.md#9-fit-扩展)、
[函数规范](../specifications/feng-function.md)、
[模块规范](../specifications/feng-module.md)和
[可见性规范](../specifications/feng-visibility.md)。以下 TODO 是这些规则的测试验收条件，不新增语言规则。

重点补齐当前内建类型／数组目标与具名目标之间的契约检查缺口，以及“关系声明”和“实现方法来源”
分离时的发码缺口。不能把“多条相同关系幂等”“同一列表重复项非法”和“不同实现相冲突”混为一类。
缺失成员等已能在适配声明处确定的错误必须在 Semantic 拒绝，不等待生成 witness 或实际调用。
...
已有的 spec 泛型转接层；静态投影字段与收窄读取的批准范围见 §28.1.3.3，其他增量仍须人工确认。
最新已确认的描述符投影方案见 §28.1.2，
它不改变默认零值、值复制、ARC 或公开函数 ABI，但涉及私有描述符结构和 `.ft` 投影依赖；
其格式迁移及具体成本门槛见 §28.1.3，不把方案方向确认当作兼容性或性能已经验收。
...
- Semantic：[`test_semantic.c`](../../test/semantic/test_semantic.c) 的
  ...
  `test_nested_union_first_exact_path`（本轮按 018 批准迁移）、`test_nested_union_cycle_rejected`，以及
...
- 2026-09-08 人工确认：union 约束的共享泛型体按联合主规范 §3.12／§3.13 验收完整收窄能力；
  COMPOSITE37 专项覆盖逐级绑定后继续匹配与 `->` 链式匹配，不能仅修复单层标签便交付。
  准入检查已按后续人工决定与普通联合绑定复用，见 §28.1.3.1；承载方案按以下结论更新。
- 后续人工确认的[共享体联合收窄设计](./feng-generic-union-match-draft.md)采用
  `reified_union_projections`：类型／聚合／函数三种上下文描述符各增加一个可选表指针，
  按投影依赖的归属读取，不按泛参声明归属读取。归属内按规范化开放身份稳定排序，闭合后不重排
  槽位；沿用既有 callable 依赖和描述符通道，不再新增独立投影隐藏参数。
  `FengGenericParamDescriptor`、方法级泛参传递、实际 `T` 的布局和默认值不变。
  字段及表项只在该设计文档定义；COMPOSITE38／39 覆盖承载契约。该字段所需私有 ABI 变更、
  `.ft` 契约识别及相关制品重建已获批准；范围和成本边界见 §28.1.3。
- 后续人工强调：共享体调用共享体时，描述信息必须沿现有机制逐层传递，使每层均可合法收窄；
  同时保障非共享体的普通 union 收窄和无 union 共享体继续正确。新增 COMPOSITE40～42 独立
  验收这三项，不仅在方案中描述，也要有实现、FCTS 行为和 compiler 证据；不改变已定准入边界。
...
三个既有问题的修复目标、复合 spec 泛型能力、准入复用及投影字段所需迁移已确认；2026-09-08
人工已批准开始 G24 实施，先执行无改动基线，再逐项修复和补测。静态元信息、收窄读取及限定场景的必要绑定构造入口
调用均已批准，见下节。实施中问题按第 30 节先记录、再分析、再解决；既有测试迁移仍需逐项批准。

#### 28.1.3 开工评估与批准边界

2026-09-08 后续 Review：原三项待决策问题均已确认。准入复用联合绑定检查、字段所需迁移，
以及静态描述信息、收窄读取和限定场景的一次必要绑定构造调用均已批准，G24 可开始实施。
整组范围与 ISSUE-G24-006 获批暂缓的边界不变；后续人工已发出开始实施及完成后全量回归的指令。

##### 28.1.3.1 联合约束准入与开放转传

人工已确认：是否满足联合约束，与普通联合类型绑定的满足规则一致，复用其编译期检查。
规则已收敛至[联合主规范 §3.12](../specifications/feng-union-type.md#312-union-form-可作为泛型类型参数约束)，
不另建泛型专用转换规则。以下为应补测的合法程序，不是已经通过的实现结果：

```feng
spec Inner: i32 | string;
spec Outer: Inner | bool;
func accept<T: Outer>(value: T): T { return value; }

// 合法：静态 i32 值可按普通联合绑定规则沿 Inner 进入 Outer。
func indirect_leaf(value: i32): i32 { return accept<i32>(value); }

// 合法：来源约束允许的实际类型均可按同一规则满足 Outer。
func forward<T: Inner>(value: T): T { return accept<T>(value); }
```

```feng
spec Named {}
type Item: Named {}
spec NamedOrBool: Named | bool;
func accept_named<T: NamedOrBool>(value: T): T { return value; }

// 合法：Item 可按已声明名义关系经 Named 进入目标联合。
func converted_member(value: Item): Item { return accept_named<Item>(value); }
```

COMPOSITE23／27 按该规则配对普通绑定与约束准入，覆盖合法、多路径顺序、无路径和错误闭合实例，
并验证开放转传证明包含来源约束自身作为实际类型的情形。只复用编译期资格与路径事实，不在
泛型调用处物化目标 union；字面量贴合仍由原有推导／目标类型规则处理。原三类边界不再待决策。

##### 28.1.3.2 私有描述符与 .ft 迁移

人工已批准三种描述符增加 `reified_union_projections` 字段所涉及的私有 ABI 变更及必要的
`.ft` 契约识别、相关制品重建；不再作为尚未批准的字段方案阻塞项。实施清单和验收见
[设计 §7.5](./feng-generic-union-match-draft.md#75-真实跨包与-abi格式边界)，版本／格式定义仍归符号表主规范。
当前 reader 检查 major、不拒绝更高 minor，不能只提高 minor 就声称旧工具链会安全拒绝。
迁移必须阻止不兼容的旧、新描述符制品混用；不引入无关的包格式重构、公开 C ABI 变更或兼容桥。

##### 28.1.3.3 成本验收门槛

人工已批准：在既有编译期具化点生成静态描述信息，增加 `reified_union_projections` 字段，
供需要联合收窄的共享体读取。表项只读，沿既有描述符机制传递；不在每次调用时动态构表，
不增加独立隐藏参数、统一包装或运行时约束搜索。该批准不是“只有静态体积、没有运行时读取”。

人工进一步批准设计 §6.2 的必要绑定构造入口调用：实际 T 的存储没有目标中间 union／spec
完整表示时，命中且需要绑定的分支通过静态表中的函数指针调用一次入口，构造正常绑定值。
每次必要绑定形成只调用一次；直接取值、无绑定或未命中分支不增加该调用，直接链式到叶子
不构造未绑定的中间 union。该批准不改变实际 T 或其默认零值，也不能用“普通绑定本来需要
构造”隐去这一次新增间接调用的成本。

人工后续在 ISSUE-G24-015 批准仅因新增 `reified_union_projections` 字段引起的已有自动
描述符字节、初始化量和栈布局增量；保留该路径，不追加开放数组静态依赖改造。除此以外，
自动生命周期描述符初始化或闭包捕获如果实际增加运行时成本，仍应先记录、分析再交人工决定。
COMPOSITE36／39 按设计 §8.2 分别核对静态数据、运行时读取、必要构造及
同包／真实跨包证据；不能依赖同包内联掩盖跨包成本，也不能为规避成本删减已确认的收窄能力。

### 28.2 用例 TODO

2026-09-09 按[逐项验收证据](./feng-language-conformance-coverage-g24-evidence.md#2-composite0143-对应证据)
核对：COMPOSITE01～43 均已完成；最后的 COMPOSITE27 已补齐约束投影转传及独立正反、
真实跨包和生命周期用例。最终沙箱外完整 `make test` 退出 0，两阶段 FCTS 均 1356／1356，
§28.3 的整组交付验收已完成，见[最终记录](./feng-language-conformance-coverage-g24-evidence.md#47-最终完整交付)。

#### 28.2.1 原有用例细化

- [x] COMPOSITE01：union 声明分别覆盖未知 member、`void` member 和 intersection member；
...
- [x] COMPOSITE02：intersection 的组成项分别使用普通 type、内建类型、数组、callable spec、
...
- [x] COMPOSITE03：源值既非目标 union 的精确 member，也不存在规范允许的进入路径时必须拒绝。
  初始化、重新赋值、实参、返回、字段写入和数组元素写入分别建立正反例；反例只改变源类型或
  目标 member，使失败确属进入资格，不混入绑定不可写等其他错误。泛型约束准入另见 COMPOSITE23。
- [x] COMPOSITE04：进入 intersection 视角必须名义满足其全部组成契约，分别覆盖全部满足、只缺
...
- [x] COMPOSITE05：union 未收窄时分别拒绝字段读写、方法调用、方法值形成及 `==`／`!=`；即使
...
- [x] COMPOSITE06：在 FCTS 为两种 form 的合法声明、进入、参数传递、返回和成员使用提供最小运行
...
- [x] COMPOSITE07：声明语法分别覆盖缺失 `|`／`&` 后的项、缺少结束分号、错误块体、企图添加
...
- [x] COMPOSITE08：union 直接成员去重后保留首次出现顺序，嵌套 union 保留为一个直接 member，
...
- [x] COMPOSITE11：按联合主规范 §3.8.1 验证多个精确路径、多个可接纳 object-spec member、
  父／子 spec 同时可达时的首路径选择；分别交换声明顺序、改变嵌套深度，断言先完成整图精确
  轮次，再进行满足轮次，各轮均为广度优先且同层左侧优先。覆盖深层精确优于浅层转换，以及
  两轮均无合法候选的反例。配对显式中间 union／spec 值选择另一条路径的正例，断言真实 tag、
  payload 与 spec 方法行为；普通绑定和泛型准入应一致，不能保留旧的多路径歧义断言。
- [x] COMPOSITE12：字面量与纯字面量常量表达式进入 union 时按叶子目标的既有贴合规则选择资格；
  覆盖可贴合、超出范围和没有合法叶子的情况。映射既有整数／浮点及 `if`／`match`／`try` 分支
  目标测试，补齐未覆盖的直接／嵌套 union 入口；断言实际 active member 和数值，不接受整数
  member 错接浮点值。保持现有分支整体目标选择和类型推导规则；成员进入选择遵循联合主规范
  §3.8.1，不把字面量贴合扩展为已具备类型值的数值隐式转换。初始化、赋值、参数、返回、字段、
  数组元素与分支结果须使用相同源值和目标作配对，覆盖直接／嵌套、默认类型精确命中、同一叶子
  经多条路径可达、较早候选越界但后续合法、全部越界及整型／浮点纯常量表达式。
  重载候选在首参数数值可贴合、后续参数不匹配时，不能污染最终候选的源类型或完整路径；
  交换候选声明顺序及嵌套调用后结果保持一致。另以实际重载冲突反例验证成员的首路径规则
  没有被扩展为“首个函数重载优先”。父／子 spec 的具体值与显式子 spec 视角按 COMPOSITE11 配对。
  实施发现嵌套数值叶子尚未进入目标选择；修复与人工批准的两轮规则记录于
  [ISSUE-G24-018](./feng-language-conformance-coverage-hardening-issues/g24.md#issue-g24-018嵌套联合的数值字面量目标选择尚不完整)，
  入口不一致独立记录为 ISSUE-G24-019；同包与真实 `.ft` consumer 均需验证，不以明确类型变量
  的进入成功代替字面量格子。旧用例仅按问题记录内批准的迁移清单调整。

#### 28.2.3 联合匹配、收窄与转换边界

- [x] COMPOSITE13：单 member 的默认／显式 `let` 绑定和 `var` 绑定取得正确 member 类型；无绑定
...
- [x] COMPOSITE14：多 member 分支绑定得到对应子集；仍有多个 member 时直接访问、调用或比较
  必须拒绝，继续绑定收窄到单 member 后合法。`else` 的剩余集合分别覆盖一个和多个 member，
  验证原变量仍保持原 union 类型，只有显式匹配的新绑定取得 member 视角；`else` 不引入绑定。不得把块式 `else` 的剩余集合规则套用到
  infix match 条件的 `else`，后者的绑定可见性由 COMPOSITE17 验证。
- [x] COMPOSITE15：普通 union match 标签分别覆盖非 member、未知类型、值／区间标签混入、
...
- [x] COMPOSITE16：嵌套绑定 match 与既有 `->` 链式匹配取得相同叶子和 payload；同首段下不同
...
- [x] COMPOSITE17：union 的 infix match 在 `if`、`while`、`&&` 右侧及普通布尔值位置分别检查
...
- [x] COMPOSITE18：union 的 object-spec 叶子进入时须先完成其既有向上转换，收窄后可按完整
...
- [x] COMPOSITE19：嵌套 intersection 展平并去重，包含各 object member 的传递父方法；共享父
...
- [x] COMPOSITE20：合并后的实例 `let`／`var` 字段和经 `T: Intersection` 访问的静态字段、
...
- [x] COMPOSITE21：intersection 值、受交叉约束的 `T` 实例以及 `T.method` 静态入口分别在明确
...
- [x] COMPOSITE22：交叉成员的访问权限按 requirement 原声明 spec 判断；普通函数、无关 type／
...
- [x] COMPOSITE23：对 `T: Union` 与 `T: Union<A>` 分别建立准入矩阵，区分精确具体 member、
  嵌套 union member、整个约束 union 类型值、仅沿嵌套路径可达的叶子、仅能转换到某个 spec
  member 的类型，以及完全无关类型。精确 member、嵌套 union member、同一完整闭合 union
  自身合法；间接叶子和经 spec 转换的类型按联合主规范 §3.12 复用普通绑定的编译期资格，
  配对 `let target: Union = source;` 与相同 source 静态类型的泛型调用检查一致性。无路径、
  错误闭合 member 实例均拒绝；多条有效路径按联合主规范 §3.8.1 选择，并与普通进入对照。
  非泛型／泛型约束、显式实参与可唯一推导实参均核验，确认没有仅为准入而包装实际 T；
  字面量先按既有规则确定实际类型，不用一次字面量贴合替代该类型的资格证明。
  type／spec owner 入口由 COMPOSITE28 配套。
- [x] COMPOSITE24：联合约束函数体内，对 `value: T` 未收窄的字段读写、方法调用、方法值、
  ...
  逐级／链式收窄的独立行为矩阵由 COMPOSITE37 验收，不用单层 match 成功代替。
- [x] COMPOSITE25：对 `T: Intersection` 与 `T: Intersection<A>` 分别覆盖全部名义满足、
...
- [x] COMPOSITE26：受交叉约束的泛型体通过 `T` 直接使用合并后的实例字段／方法及静态成员，
...
- [x] COMPOSITE27：复合约束从仍开放的泛参向其他泛型函数、type／spec owner 和实例／静态方法
  继续转传时，分别验证合法的相同／更强约束，以及无约束、较弱或无关约束的拒绝；owner 泛参与
  方法泛参不同名且正确替换，具体闭合后不得串到另一实例。联合约束按主规范 §3.12 复用
  绑定资格及约束蕴含检查，覆盖 §28.1.3.1 的合法嵌套转传，并将来源约束自身也作为实际类型
  测试；补齐部分来源类型不满足或只有 member 集合相似的反例，并验证新增合法路径不再因旧
  多路径歧义而阻断开放转传。错误须在泛型声明
  使用点检出，不能靠当前调用点暂时只传合法类型漏过；不新增不同泛型实例间的 variance。
  051 的交叉约束转传须复用约束 witness 的组成路径并保留实际 T，分别覆盖类型级／方法级
  泛参及多参数槽序；成员声明初始化同时覆盖 `inner<T>(...)` 与 callable value `inner<T>`。
  内层返回闭包和成员保存后的调用必须验证描述符生命周期，不能只用不逃逸的同步调用验收。
  同包／真实跨包、引用／值／完整 spec 实参及正常／异常清理配对；新增描述符字段或成本仍
  先交人工 Review，参见 [051 续作核对](./feng-language-conformance-coverage-hardening-issues/g24.md#051-续作范围与描述符生命周期核对2026-09-09)。
  字段的默认值和用途边界按 [G24 约束投影字段](./feng-generic-optimize-dev.md#g24-约束投影字段)
  验收：无投影依赖为 `NULL`、无关共享路径不读取该字段、投影后实际 T 的 kind／类型描述符
  保持不变；字段定义、生成、读取处的注释须与该边界一致。
- [x] COMPOSITE28：复合 spec 自身带泛参及 owner 约束时，分别验证成员类型正确替换、合法实例
...
- [x] COMPOSITE29：复合值经绑定、参数、返回、字段／数组存储及收窄后，保持各具体 member／
...
- [x] COMPOSITE30：默认 union 选取去重后首个直接 member，嵌套时保留各层默认 member；显式
...
- [x] COMPOSITE31：provider 导出真实 package-public `.ft` 后释放源码 AST，再由独立 consumer
  ...
  优先复用；同包多文件顺序变换不得冒充跨包 `.ft` 验收。投影依赖还须验证 owner、开放类型
  引用、路径和绑定用途完整恢复；缺失必需记录、错误 owner／引用及不兼容契约按已批准的迁移
  方案拒绝，不能恢复出错误槽位后继续发码。槽位与表项分别配套 COMPOSITE38／39。
- [x] COMPOSITE32：在 Codegen 测试核对 union 的完整静态进入路径、叶子转换、已绑定 member 的
  ...
  无新增开销，不把既有 tag 写入、值复制或必要 ARC 误列为新增成本。联合共享体需分别覆盖
  match 语句、值表达式、全部分支终止的表达式和 infix 四个发码入口，不能只修复其中一个。

#### 28.2.7 联合与交叉作为泛型类型实参

- [x] COMPOSITE33：无约束 `identity<T>` 及泛型 owner 的实际 `T` 分别为完整 union、完整
...
- [x] COMPOSITE34：对 `T: Union`，同一泛型声明同时以约束 union 自身、合法直接 member 和
  ...
  ISSUE-G24-004 的收窄发码修复，也不能为解决发码将合法 match 改成前端报错。该承载矩阵同样
  应用于 COMPOSITE37 的逐级／链式收窄；不能只在参数本来就是完整外层 union 时支持多级匹配。
- [x] COMPOSITE35：实际 `T` 为交叉 spec 时，覆盖相同交叉约束及其组成 object spec 约束；
...
- [x] COMPOSITE36：在同包及真实二进制跨包中，以本组变更前的同一合法程序及同类已支持路径
  ...
  ISSUE-G24-006 只延期消除优化，不豁免正确性和增量成本验收。按 §28.1.3.3 对已批准的
  静态元信息／读取与限定场景的一次构造入口调用分别核对，不以一般 spec 的必要操作名义隐藏
  间接调用。描述符结构和 `.ft` 的必要迁移按 §28.1.3.2 已批准范围实施并验收。

#### 28.2.8 共享泛型体内的联合逐级与多级收窄

- [x] COMPOSITE37：依据联合主规范 §3.12／§3.13，以普通 union 参数函数为行为对照，在
  `T: Union` 及 `T: Union<A>` 的共享泛型体中分别使用“绑定内层 union 后继续 match”和
  `->` 链式路径；断言两者命中相同分支、取得相同叶子类型及非零 payload。覆盖两／三层、
  同首段下不同叶子、命中与不命中／fallback，深路径配套 COMPOSITE10 的 8／9 段边界。
  `match` 语句、块式表达式及 infix 形式分别提供适用的有绑定／无绑定证据；用带计数副作用的
  target 核对单次求值，并确保非目标分支不能静默通过。实际 `T` 按 COMPOSITE34 区分完整
  外层 union、合法内层 union member 和非 union 的直接 member；内层 union 激活不同叶子，
  引用、trivial 值及含受管字段的值按 COMPOSITE29 验证身份、副本与存活。中间层绑定还须
  经正常返回／存储后继续匹配，不能只验证不离开原分支的叶子读取；各入口显式实参／推导及
  方法 owner 泛参的代表场景均有证据。无绑定分支访问原始 `T` 的成员、错误首段、非 union 中间层、错误
  末段、越级／兄弟路径及从 object-spec 下探具体实现的反例，按 COMPOSITE13／COMPOSITE16
  配对到共享体并在 Semantic 拒绝；不能只在实例化后由 Codegen 兜底。间接叶子作为原始 `T`
  是否准入仍按 COMPOSITE23 单独决定，不因多级匹配成立自动扩大准入。真实跨包按
  COMPOSITE31 验证；COMPOSITE32／COMPOSITE36 核对实际承载、取值路径与成本，不得读取裸
  `T` 中不存在的 union tag／payload，不得以统一重新包装或新增分派层绕过开销验收。

#### 28.2.9 投影描述符承载与稳定槽位

- [x] COMPOSITE38：按[共享体联合收窄设计 §7](./feng-generic-union-match-draft.md#7-元信息通道描述符的-reified_union_projections)
  验证投影依赖归属及槽位。引用／值 owner 的字段初始化、构造和允许的终结器／静态初始化
  分别使用类型／聚合描述符；顶层函数、实例／静态方法和 fit 方法使用各自函数描述符。
  同一方法同时使用 owner 的 T 与方法的 U 时，两者的投影均在方法依赖域，不能按 T 的声明
  位置误读类型表。无直接投影使用的表为空，有投影使用时槽位完整；callee 只有投影依赖的
  开放转传不能漏建 callable 依赖。以两个闭合实例、文件／收集顺序变化、真实 `.ft` 恢复，
  以及开放类型闭合后相同或路径静态不匹配的实例，验证槽位不删减、不重排、不串 owner。
  各类别索引独立，不能与 aggregate／type／callable 槽混用；新增精确 compiler 测试并配对 FCTS 结果。
- [x] COMPOSITE39：按设计 §5／§6／§8 验证投影表项的生成和消费。静态不可能路径不读取源值；
  静态确定层不读取不存在的 tag；真实 tag 仅在父层命中后读取；直接绑定使用正确偏移并保持
  普通绑定的值语义。直接取值、无绑定或未命中分支不得调用 materialize；需要构造缺失的
  完整绑定值时，只在该绑定形成处调用一次并直接初始化目标，不先请求默认零值，目标大小／
  清理按绑定结果类型处理。链式直接到叶子不得构造未绑定的中间 union。
  生成 C 验证表为编译期静态数据、不增加投影隐藏参数、不迁移方法级泛参、不修改 witness
  用途；开放转传／方法值／闭包沿现有描述符通道，离开形成作用域后仍有效。
  对照普通 union、直接 leaf 及非投影共享调用，记录静态和临时描述符体积／初始化、实际表项
  读取和必要构造入口的增量；与 COMPOSITE29／36／37 的生命周期及同包／跨包行为证据配对。

#### 28.2.10 多层共享调用与既有路径不回归

- [x] COMPOSITE40：以至少三层共享调用验证投影依赖逐层可用。先使用已确定合法的相同联合
  约束，在每层分别执行不同的合法收窄路径，并通过独立的结果或计数断言证明每层均正确命中、
  取得完整 payload；不能只在最外层或最终返回值中断言。实际 T 覆盖合法裸 member、完整 union
  和内层 union member；同一静态 union 的不同 active member 分别运行。
  另配中间层不直接收窄、只转传给内层的程序，验证其空投影表不会切断 callee 描述符依赖。
  顶层函数、实例／静态方法及类型初始化发起的调用链分别映射适用证据；至少包含 owner／方法
  泛参替换、双泛参调换、两个闭合实例连续调用及有明确退出条件的递归，防止串表、串槽或沿用
  上一次实例的元信息。同包三层链真实运行；跨两个包边界的三层链通过真实 `.ft`／二进制消费
  验证。更强／更弱约束的转传资格另按 COMPOSITE27，不在本项新增关系。test/ 检查 callee
  描述符沿现有 reified_callable_deps 机制正确取得或复用，各收窄点读取所属上下文的投影槽；
  不新增传递协议、动态构表或投影专用隐藏参数。FCTS 验证逐层分支、返回 T、引用身份及值／
  受管 payload 生命周期，复用 COMPOSITE29 的观察方式。
- [x] COMPOSITE41：非共享体的普通 union 收窄建立独立不回归证据，覆盖普通 union 与已闭合的
  泛型 union 值，配对单层、逐级、链式、多 member 子集／else、infix，以及语句、值表达式和
  全分支终止的适用入口。保留目标单次求值、匹配顺序、正常绑定值语义和 return／throw 清理；
  非法标签、非法路径及未收窄使用仍由原有前端规则拒绝。优先映射 COMPOSITE13～18、29 的
  已有正反例，缺少直接证据才新增独立测试。生成 C 在相应非共享函数体内仍直接按确定的
  tag／payload 处理，不增加投影读取、构造入口间接调用或包装；与 FCTS 同包／真实跨包结果
  配对。这里不禁止 ISSUE-G24-002 等已批准缺陷的修复，但不得借共享体补齐改变普通 union
  的语义、合法性边界或正常路径成本。
- [x] COMPOSITE42：完全没有 union 使用的共享函数／类型／方法建立控制用例，覆盖无约束、
  object／intersection／callable 约束的适用调用，及 aggregate／managed type／callable 三类
  具体化依赖。验证 owner 与方法泛参、开放转传、返回 T、字段／数组存储、默认零值与显式初值、
  方法值／闭包、引用身份和含受管字段的值生命周期；以既有直接证据优先，不重做其他专项。
  另配 union 共享路径调用不收窄的通用共享 helper，再返回联合调用方继续收窄的混合链，验证
  空投影表不破坏原有泛参／依赖转传；以及同一共享方法中联合泛参与非联合泛参并用，防止新增
  投影槽污染既有 witness 或其他依赖槽。test/ 核对无直接投影依赖的表为空、对应函数体不读取
  投影、不新增空表判断或分派；FCTS 在同包和真实跨包验证实际行为。静态／临时描述符初始化
  及成本配套 COMPOSITE36／39；既有错误程序保持既定诊断规则，不把缺少投影的内部空表当作
  用户错误，也不把已批准修复的旧缺陷当作必须保留的行为。不得修改旧测试来掩盖回归。

### 28.3 独立验收与交付 TODO

- [x] COMPOSITE43：按 [spec 主规范第 4 节](../specifications/feng-spec.md#4-语义)补齐交叉值向组成
  spec 的显式转换。覆盖左右成员、嵌套交叉成员、成员的 object 父级、精确泛型实例、同包与
  真实 `.ft` 跨包、引用与装箱值、默认零值、可变字段、方法值、单次求值和生命周期。
  反例覆盖对应隐式赋值／参数／返回／字段／数组写入、反向／无关转换、不同泛型实参及 union；
  配对保留 object-spec 子到父的既有显式和隐式行为。Semantic 检查路径与诊断，Codegen 检查
  静态 witness 指针及固定路径读取，FCTS 验证运行结果；不扩展泛参或函数描述符，051 的
  开放泛参跨约束转传另行处理。实施与验收见 [ISSUE-G24-052](./feng-language-conformance-coverage-hardening-issues/g24.md#issue-g24-052交叉类型值缺少向组成-spec-的显式投影)。

- [x] 按 §28.1.2 的已批准范围执行，剩余不确定边界以具体程序提交人工决定；逐项列出“主规范—TODO—既有／新增测试—
  断言”的映射，每项未新增的原因必须是已有直接证据或已批准的不适用结论，而不是仅有类似用例。
- [x] 对 COMPOSITE01～COMPOSITE43 按各项语义的适用性核对正向／反向、声明／使用、同包／真实
  跨包、显式／推导、实际 `T` 为具体类型／spec、引用／值及边界组合；每个适用格记录复用或新增
  的直接证据。缺少运行断言、尚未复现或仍待决策的格子不得算完成，不以“全量通过”掩盖未覆盖规则。
- [x] 建立具体诊断模板到源码可达入口的映射，优先核对 `AE0601`～`AE0607`、`AE0621`／`AE0622`
  及相关通用类型、约束、调用、可见性错误。`AE0622` 当前同时用于非法 union member 和交叉
  满足失败，须区分根因，不能只覆盖其一；`AE0608` 按 018 批准停用并保留历史编号。
  Parser 已阻断或仅内部防御的出口不强造用户反例。
- [x] 非法用例核对完整诊断集合的码、文件、token、行列、数量与阶段；避免依赖错误列表顺序或
  未承诺稳定的完整文本。所有正例检查无错误，FCTS 必须真实运行，并通过明确结果或失败分支
  排除“没有进入断言分支也通过”；仅 Semantic 接受不能替代行为验收。
- [x] 既有测试默认全部保持原样；新增独立最小用例补足薄弱断言。确需修改旧例时，在问题记录
  明列文件、函数／测试块、原断言、拟改动和原因，经人工再次批准后才能修改，不概括迁移错误码。
- [x] 独立执行 G24 的 Parser／Semantic／Codegen／Symbol 专项及 FCTS 行为，逐项记录同包、
  真实跨包、合法／非法、值／引用及两类泛型约束的适用证据。补测发现编译缺陷先记录、再分析，
  取得对应批准后修复；不合并与本组用例无关的重构。
- [x] 单列 COMPOSITE40～42 的多层转传、非共享 union 和无 union 共享体结果；既有测试通过
  不能代替未覆盖边界，新增共享收窄通过也不能代替两类既有路径的不回归证据。
- [x] 在 Codex 沙箱外为 G24 独立执行 `make test`；
- [x] 核对运行时成本和 ABI／格式边界，执行 `git diff --check`，关闭或取得不阻塞交付的明确
