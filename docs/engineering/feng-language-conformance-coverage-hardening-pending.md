...
MOD07 会有意终止进程，只使用隔离运行结果核对，不接入常规 FCTS 可执行入口，避免破坏其他用例的
独立运行与全量回归。
...
- 实际文件与用例：
  ...
  - 既有 `test_generic_static_binding.ff`：`cross-package generic static initializer runs once` 与 aggregate
    ...
    存储隔离和 MOD13 可变状态隔离；
  - MOD07：隔离运行原最小用例，未接入常规 FCTS 入口。
- 本组专项结果：`make fcts-tests` 通过，942/942；MOD07 隔离运行收到 signal 11，符合当前平台自然耗尽
  调用栈的表现
- 本组沙箱外 `make test`：通过；UBSan 与 normal 全量阶段均完成，FCTS 均为 942/942，性能约束、增量
  构建与发布脚本检查通过
...
- 实际变更：模块主规范已改为自然栈溢出；MOD07 从常规 FCTS 运行组移除，保留隔离运行事实作为验收
  记录。

##### 验收与关闭

- 本组专项复验结果：隔离执行 MOD07 时进程收到 signal 11，符合当前平台自然耗尽调用栈的表现；移除
  该故意崩溃用例后，常规 `make fcts-tests` 通过，942/942；
- 本组沙箱外 `make test` 结果：通过；UBSan 与 normal 全量阶段均完成；
- 关闭依据：模块主规范、早期设计和当前实现已经收敛一致，MOD07 隔离事实、常规 G01 专项及全量
  回归均通过。
