...
> 状态：Review 中；G01 可开始实施，其余组待 Review
>
...
验证模块级绑定和类型 static 绑定在首次读写、依赖和初始化循环中的惰性初始化行为。

预期新增 `test_module_binding_semantics.ff`、必要的最小 `fcts_lib` 提供端文件和主入口登记。

### 5.2 用例 TODO

- [ ] MOD01：同文件模块绑定首次读取只初始化一次；
- [ ] MOD02：模块绑定首次直接写入前完成初始化；
- [ ] MOD03：模块绑定复合写入前完成初始化且只执行一次；
- [ ] MOD04：声明顺序相反的模块绑定依赖按实际读取初始化；
- [ ] MOD05：同模块多文件绑定依赖保持同一初始化语义；
- [ ] MOD06：跨模块或跨包首次读取触发目标绑定初始化；
- [ ] MOD07：初始化循环读取尚未完成的绑定时观察其类型默认值；
- [ ] MOD08：static `let` / `var` 复用模块级绑定的延迟初始化规则，重复访问同一静态绑定只初始化一次；
- [ ] MOD09：static 绑定读取另一 static 绑定时按依赖初始化；
- [ ] MOD10：static 绑定读取模块绑定时按依赖初始化；
- [ ] MOD11：同一泛型闭合类型实例的 static 绑定重复访问时只初始化一次；
- [ ] MOD12：同一泛型类型的不同闭合类型实例分别拥有独立 static 存储和初始化状态；
- [ ] MOD13：修改一个泛型闭合类型实例的 static `var`，不影响其他闭合类型实例的对应绑定。
...
初始化循环只断言规范定义的默认值观察，不推导多个初始化器之间未规定的相对顺序。

### 5.3 独立验收与交付 TODO

- [ ] 核对模块、绑定和类型规范，并确认 MOD01～MOD13 没有等价直接证据；
- [ ] 独立运行 G01，核对初始化次数、跟踪顺序、默认值和最终值；
- [ ] 在 Codex 沙箱外为 G01 独立执行 `make test`；
- [ ] 执行 `git diff --check`，关闭或决策 G01 问题；
...
- 本组专项结果：—
- 本组沙箱外 `make test`：—
- 问题：`ISSUE-G01-001` 待通过 MOD11～MOD13 验证，不阻塞启动
...
- 本组专项复验结果：待 G01 实施后填写；
- 本组沙箱外 `make test` 结果：待 G01 实施后填写；
- 关闭依据：MOD11～MOD13 通过，且 G01 独立全量回归通过后关闭。
