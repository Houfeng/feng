...
验证底层表示固定为 `i32` 的 enum 声明、成员和值关系产生的稳定 Parser / Semantic 诊断。enum
宽度不得随平台 `int` 别名变化；显式成员值必须位于 `i32` 可表示范围内。

### 23.2 用例 TODO

- [ ] ENUM-D01：enum 成员重复声明；
- [ ] ENUM-D02：非整数字面量及字面量表达式继续映射 Parser `SE0408`；全显式 enum 的任一成员值小于
  `-2147483648` 或大于 `2147483647` 时，必须在该成员名称 token 产生唯一 Semantic enum 段诊断
  `AE0405`，不得在 Codegen 截断，也不得延迟到 `.ft` 导出失败；`i32` 最小值与最大值必须合法；
- [ ] ENUM-D03：不同 enum 类型之间错误赋值；
- [ ] ENUM-D04：同一 enum 值显式转换到全部有符号、无符号整数类型及平台别名必须合法，并按目标整数
  类型的既有显式转换规则得到结果；任意整数类型到 enum、不同 enum 之间的显式转换必须在 cast
  表达式起始 token 产生唯一 `AE1023`。enum 与整数之间的双向隐式转换仍归 `AE1003`；
