...
日期：2026-09-09。状态：已实施并验收。

本文汇总 G24 后关于三张 `reified_*` 元信息表补齐 `_count` 的讨论，并作为本项实现与验收的
唯一主记录。2026-09-09 人工批准按本文实施；runtime、Codegen、测试及私有 ABI 只允许按本文
所列范围变更。
...
2026-09-09 实施前核验确认该自动路径仍存在。在 macOS arm64、同一生成 C、同一 Clang 与
`-O0` 独立调用边界下，三个 count 使该路径的 `FengTypeDescriptor` 清零长度从 184 增至
208 字节，所在函数栈帧从 464 增至 496 字节；函数机器码指令与调用数量不变。静态
`FengFunctionDescriptor` 从 120 增至 144 字节。人工确认接受这段短期成本，先独立完成本文，
随后立即开始 `T[]` 描述符静态化优化；本项不得据此合并静态化代码。
...
以下步骤已经完成；本文件不授权修改未明列并获批的既有测试，也不扩展运行时成本许可。

### 5.1 实施前核验与文档对齐

- [x] 核对三个描述符的布局、所有初始化器和三张表的生成入口，保存静态及自动记录的基线。
- [x] 确认第 4 节成本边界；仍有自动记录受影响，已记录测量与人工决定。
- [x] 实施获批后，先将相关设计文档中的当前字段描述对齐；尤其更新
  [联合投影设计](./feng-generic-union-match-draft.md) 的“不新增 count”表述，避免把新旧方案并列为现行规则。

### 5.2 字段与静态发码

- [x] 三个描述符各补齐三个 count，并注释数量含义、零值规则及“不用于运行时检查”的边界。
- [x] 三类表生成与描述符初始化使用一致的依赖数量；覆盖普通静态记录、默认／内建描述符
  及仍存在的自动初始化器，不增加运行时计算 count 的代码。
- [x] 保持共享体读取、传递、槽位排序和现有表项复用逻辑不变；不附带重命名或无关重构。

### 5.3 编译器、跨包与行为验证

- [x] `test/` 验证三种描述符上的三张表分别为零项、单项、多项时，count 与实际长度一致；
  覆盖无依赖、三表并存、闭合后相同记录复用及不可能 union 投影仍保留槽位。
- [x] 验证类型级／方法级泛参、成员绑定初始化、构造／析构、fit 与多层共享调用的依赖归属；
  caller 仅转传时不错误汇总 callee 数量。
- [x] 两种 `.ft` profile 真实往返后数量与槽位一致，consumer 不依赖 provider 源码；
  复用或补充既有数量不符、槽位越界等制品反例，不把校验移到生成程序运行时。
- [x] `fcts/` 复用或新增同包／跨包联合收窄、spec 视角形成、约束转传与非相关共享体的行为
  对照，验证返回结果、所有权与闭包逃逸；不以 count 文本断言代替语言行为。
- [x] 现有用例及断言保持不变；仅注册独立新增的 count 验证入口，无需迁移既有断言。

### 5.4 独立回归与交付

- [x] 比较生成 C 与独立调用边界的汇编：无新增 count 读取、检查、调用、ARC 或描述符逐次
  组装；记录实际静态体积、字段偏移与栈空间差异，不仅比较源码中的显式赋值。
- [x] 在 Codex 沙箱外独立执行 `make test`，记录日志与退出状态；制品在工程目录内运行。
- [x] 执行 `git diff --check`，填写实现、测试、成本和问题关闭结果，再更新交付状态。
- [x] 建议实施 commit message：`feat: add counts to reified metadata tables`；不自动提交。

## 6 实施过程问题记录

实施与验收已完成，未发现本专项新 Bug。第 4.2 节记录的是已知临时描述符路径、实测成本及
人工决定，不把该独立静态化缺口登记为本项 Bug。

### 6.1 实现结果

- runtime 三种描述符均按“count 在前、指针在后”补齐三个字段；空表继续依靠 C 初始化器零值，
  不为默认／内建描述符增加逐字段发码。
- Codegen 的 callable、引用类型及聚合值类型初始化器直接使用生成对应表的同一
  `FengReifiableDepSet` 数量，不新增数量推导、排序、去重或运行时计算。
- 共享体和 runtime 未新增 count 消费者；`.ft` 字段、格式版本及既有校验保持不变。

### 6.2 测试与回归结果

- 新增独立 Codegen 测试，覆盖三种描述符、三张表的零／一／二槽位，三表并存、不可行 union
  槽位、相同闭合约束记录占两个槽位，以及类型、方法、fit、初始化器、构造、析构和仅转传 caller。
- package-public 与 workspace-cache 两种 `.ft` profile 均经真实导出、释放 provider 分析结果、
  无源码导入 consumer、生成 C 并编译；既有制品损坏、数量不符和槽位越界测试继续通过。
- 2026-09-09 在 Codex 沙箱外执行完整 `make test`，UBSan 与普通两轮均通过，最终退出状态为 0；
  其中 smoke 为 91/91、标准库为 604/604、FCTS 为 1356/1356。
- 最终 `git diff --check` 通过。

### 6.3 ABI 与执行成本复核

macOS arm64、`sizeof(size_t) = 8` 下最终布局如下；每种描述符均净增 24 字节：

| 描述符 | union count／指针偏移 | spec count／指针偏移 | constraint count／指针偏移 | 最终大小 |
| --- | ---: | ---: | ---: | ---: |
| `FengTypeDescriptor` | 152／160 | 168／176 | 184／192 | 208 |
| `FengAggregateDescriptor` | 128／136 | 144／152 | 160／168 | 184 |
| `FengFunctionDescriptor` | 96／104 | 112／120 | 128／136 | 144 |

独立调用边界复核未发现新增 count 读取、检查、调用或 ARC，机器码指令数量与调用数量不变。
现存 `T[]` 自动描述符仍使清零长度由 184 增至 208 字节、栈帧由 464 增至 496 字节；该已批准
短期成本将在本文完成后立即启动的独立静态化优化中处理，不属于本次实现范围。
