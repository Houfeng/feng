...
- [x] 实现"全字面量"兜底：所有分支均为字面量且无明确目标类型时，各分支各自默认推导
...
- [x] 实现目标类型确定逻辑：优先取外层明确目标类型（绑定注解、函数/方法参数、返回值、成员赋值）；无明确目标类型时始终取 body 表达式的推导类型（body 是主路径，方向固定）
- [x] 实现 catch 块向 body 类型的贴合：catch 块 yield 值为纯数值字面量时，向 body 类型贴合，范围检查 + 写入 AST 节点 `type`
