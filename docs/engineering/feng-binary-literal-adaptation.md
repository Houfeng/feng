...
// 场景 C：全字面量且无明确目标类型
let w = if cond { 1 } else { 2 };      // ✅ 各分支默认推导为 i64，结果类型一致
...
// try 表达式：同样按 body、各 catch 的顺序选择目标（try 后是单个表达式，不是块）
let v = try x catch { 10 };    // ✅ body=x(i32) → 目标 i32；catch 的 10 贴合到 i32
let u = try 10 catch { x };    // ✅ catch 是第一个有确定类型的非字面量分支，body 的 10 贴合到 i32
```

**贴合原则**（已决策）：

`if` / `match` / `try` 的目标类型选择、分支顺序与逐分支贴合统一以 [Feng 语言流程控制规范 §4.1](../specifications/feng-flow.md#41-if--match--try-%E8%A1%A8%E8%BE%BE%E5%BC%8F%E7%BB%93%E6%9E%9C%E7%B1%BB%E5%9E%8B)为准；本工程记录不再单独定义规则。
...
- [x] 实现"全字面量"兜底：所有结果分支分别按默认类型推导，随后要求结果类型一致
...
- [x] 实现目标类型确定逻辑：优先取外层明确目标类型（绑定注解、函数/方法参数、返回值、成员赋值）；无明确目标类型时取第一个非字面量分支的推导类型
- [x] 实现字面量分支向目标类型的贴合：范围检查 + 贴合结果写入 AST 节点 `type`
- [x] 实现联合类型特殊处理：目标类型为联合类型时，非字面量分支的推导类型是联合类型成员即合法；字面量分支能贴合到某个成员时即合法
- [x] 实现"全字面量"兜底：所有结果分支分别按默认类型推导，随后要求结果类型一致
- [x] 改造 match 分支验证（analyzer.c:7366/7609/7819）及调用链
- [x] 新增测试用例
- [x] 全量回归测试，通过后将本任务所有 TODO 标记为完成，等后续指令

#### 10.3.4 try 表达式分支贴合

> 贴合原则见流程控制规范 §4.1。try 后是单个表达式，不是块；catch 后是块。

- [x] 实现目标类型确定逻辑：优先取外层明确目标类型；无明确目标类型时，按 body、各 catch 的顺序取第一个非字面量确定类型分支；若不存在，则各字面量分支分别默认推导
- [x] 实现各正常分支向所选目标类型的贴合，范围检查 + 写入 AST 节点 `type`
- [x] 实现联合类型特殊处理：目标类型为联合类型时，非字面量 catch 块的推导类型是联合类型成员即合法；字面量 catch 块能贴合到某个成员时即合法
- [x] 改造 try 验证（analyzer.c:6586/6636）及调用链
- [x] 新增测试用例：覆盖 body 字面量 / body 非字面量、catch 字面量贴合 / catch 非字面量不匹配、有明确目标类型等场景
- [x] 全量回归测试，通过后将本任务所有 TODO 标记为完成，等后续指令
...
- [x] 实现目标类型确定逻辑：有明确目标类型时沿用现有路径；无明确目标类型时取第一个非字面量元素的推导类型
- [x] 实现字面量元素向目标类型的贴合：范围检查 + 贴合结果写入 AST 节点 `type`
- [x] 实现"全字面量"兜底：所有元素均为字面量时，各元素各自默认推导，沿用 `inferred_expr_types_equal` 比较
- [x] 改造 `infer_array_literal_expr_type`（analyzer.c:13595）和 `validate_array_literal_expr`（analyzer.c:13625）
- [x] 新增测试用例：覆盖首元素非字面量、首元素字面量（次元素非字面量）、全字面量、非字面量类型不一致等场景
- [x] 全量回归测试，通过后将本任务所有 TODO 标记为完成，等后续指令
...
- [x] 确认现有测试覆盖元组字面量元素贴合场景（如 `let p: Point = (1, 2)` 各元素按位贴合）；若不足则补充
- [x] 全量回归测试，通过后将本任务所有 TODO 标记为完成，等后续指令
