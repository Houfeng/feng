...
[Feng 语言正确性用例补齐实施文档](./feng-language-conformance-coverage-hardening-pending.md) 是本步骤的
唯一详细实施清单。用例编号、文件、Todo、问题和验收记录均只在该文档维护。

#### 子交付 G01：惰性初始化语义

- 范围：模块级绑定、类型 static 绑定和泛型闭合类型实例的 static 存储隔离；
- 状态：已交付；
- 顺序：本步骤第一组；
- 验收：G01 专项通过，并在 Codex 沙箱外独立执行 `make test`；
- 详细范围：实施文档 G01。

#### 子交付 G02：模块名称解析语义

- 范围：import alias、全部公开顶层声明的完整模块路径、模块级绑定存储身份、惰性碰撞和文件级 import 隔离的合法行为；
- 状态：已交付；
- 顺序：G01 独立交付后实施；
- 验收：G02 专项通过，并在 Codex 沙箱外独立执行 `make test`；
- 详细范围：实施文档 G02。

#### 关联交付 G22：模块可见性诊断

- 范围：模块路径、import、alias、导出、可见性和文件级隔离的稳定 Semantic 诊断；
- 状态：已交付；
- 顺序：按实施文档的诊断组顺序实施；
- 验收：G22 专项通过，并在 Codex 沙箱外独立执行 `make test`；
- 详细范围：实施文档 G22。

G01、G02 与 G22 是三个独立交付组，专项和全量回归结果不得互相复用。
...
CLI 文本诊断的类别断言使用统一辅助函数，只匹配行首的“类别前缀 + 四位十进制数字 + 冒号和空格”，
例如 `CE0001: `；路径、源码上下文和诊断文案中出现的类别字母或错误码不计为该类别诊断。
回归须使用固定含 `CE` 的测试路径验证语义错误不会误判为 Codegen 错误，并验证实际输出的 `CE` 诊断仍能被识别。
