...
- [x] 5.11 `->` 链式多级模式语法解析（Parser）
- [x] 5.12 多级模式层级校验：每级必须是前一级类型的成员
- [x] 5.13 Pattern 编译：`->` 链多级匹配（非嵌套展开，路径级重叠检测，最深类型窄化）
- [x] 5.14 链式穷尽性检查：链分支覆盖一级成员
- [x] 5.15 补充测试用例：链式匹配解析（1/2/3 级链、绑定）、层级校验错误（AE1116、AE1117）
- [x] 5.16 全量回归测试
