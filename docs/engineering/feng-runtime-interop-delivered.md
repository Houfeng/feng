...
4. 若 helper 返回的是新构造的托管值（例如 `feng_array_slice(array, start, length)` 这类复制型数组 helper），则 helper 必须在返回前把结果构造为一个完整、可独立持有的 Feng 值；若结果中包含托管子元素，也必须由 helper 先取得这些子元素所需的独立持有权，而不是把“仍借用输入参数中的元素引用”暴露给调用方。
