...
- Lambda 调用入口的函数定义与前导代码（闭包环境转换、泛型上下文恢复）统一锚定到 Lambda 定义行，再由参数和函数体各自建立源码映射；不得继承前一个 callable 结束后的 `<feng-generated>` 映射或按生成 C 的行数递增到无关源码行。步入 Lambda 的首次停顿必须对应真实 `.ff` 源码，步过和步出继续使用原生单步语义。
...
- `feng dap` 现已在 `stackTrace` 上隐藏标记为 runtime / generated helper 的 `HIDDEN` frame。
- `frame_policy` 的 `COLLAPSE` 语义继续在后续子项叠加，不在本子切片中同时展开。
- 对纯 runtime / generated helper frame 默认隐藏，必要时可提供开发者模式开关显示原生 frame。

#### 源码单步完成后的停顿

- `feng dap` 根据 [feng-cli.md §2.2](../specifications/feng-cli.md#22-feng-dap) 的隐藏帧单步规则，在转发 `stopped` 前异步查询原生栈，并复用后端的执行请求；栈展示过滤本身不能替代停顿处理。
...
- Lambda 创建与调用的断点回归在该 C 入口中独立新增，覆盖紧邻创建行的函数体首行、外层后续语句、循环重复调用、未调用的闭包、捕获、嵌套、泛型上下文以及直接返回和地址传出返回值；同时校验断点顺序、次数和所属栈帧，避免仅凭行号通过错误停点。
- 整数表达式与 Lambda 变量读取使用独立新增的回归用例，覆盖加减乘、复合赋值、嵌套及短路表达式、循环和递归停顿，以及按值、按地址传入的参数、参数修改、捕获和外层同名变量。真实 DAP 会话同时验证 locals、hover、watch 和只读表达式的值；编译器测试检查调试读取表达式及内部临时变量标记，运行时语义和性能约束沿用原有全量流程。读取和停顿语义以主规范为准。
- Lambda 单步使用独立新增的真实 DAP 用例，覆盖无捕获、捕获、嵌套、表达式体、泛型函数和泛型方法中的 Lambda，以及按值和按地址的调用 ABI；从调用行断点验证首次步入的源码、继续到函数体后的参数与捕获值、步出回到调用方，以及步过直接到调用后的源码行。编译器测试同时检查入口前导代码的逻辑源码映射和关闭调试行映射时的生成代码一致性，映射规则以 §5.9 为准。
- 程序退出单步使用新增真实 DAP 用例覆盖末行断点、三种源码单步、普通函数返回、非零退出码及收尾析构断点；独立协议用例覆盖隐藏帧与调用方判定、非单步停顿、指令粒度、请求失败及异步取消，行为规则以主规范 §2.2 为准。
