...
上述规则只作用于 callable 的显式参数，不改变值类型实例方法的隐式 `self`。显式值参数进入当前
callable 时先按值语义建立参数存储；随后在该参数上调用 `@value`/tuple 方法不会再次复制接收者，
隐式 `self` 直接引用该参数存储，方法内成员修改因而回写当前参数。若直接在调用方变量上调用同一方法，
`self` 则直接引用该变量的存储并回写该变量。
...
动态布局与固定布局开放值都遵守同一规则。调用方按 §7.17 在进入 callee 前建立本调用边界的独立参数
存储，callee 通过 origin 声明视角读取该存储；callee 随后在参数上调用实例方法时不会再次复制接收者。
闭合值类型与非泛型函数仍保持原来的直接 C ABI。固定布局值原本就必须完成一次
...
泛参的 `@value`/tuple 实例，其 C 表达式已经表示当前参数存储的地址。旧的实例方法调用路径只识别部分
descriptor-sized 存储，仍会对固定布局开放值再次取址。例如 `Span<Composite<T>>` 参数调用 `slice` 时
错误生成指针载体自身的地址，而不是当前 Span 参数的存储地址，callee 因而读取无效字段并抛出边界异常。

通用修复规则是：所有值类型实例方法的隐式 `self` 始终引用当前接收者存储。普通值表达式在调用处取址；
表达式结果已经标记为存储地址时直接复用，不得再次取址。该规则统一覆盖类型自身方法、用户类型 `fit`
方法、方法级泛型方法、开放泛型实例的共享方法及共享体内的 self-call。它不改变显式参数的按值复制
规则：显式值参数的独立存储在进入当前 callable 时已经建立；随后以该参数为接收者调用方法时不再复制，
`self` 直接引用并修改该参数存储。若直接以调用方变量为接收者，`self` 直接引用并修改该变量。修复不修改
runtime API/ABI，不增加复制、分配、描述符读取或运行时分支，只纠正生成的 C 地址表达式。
