...
- `@mixable` wrapper 在完整 Semantic 检查前展开并进入普通成员表，此后不存在独立的重载决议路径，自然使用普通静态方法或实例方法的规则。
...
最后一项不是独立的 `@mixable` 重载决议。`@mixable` 在完整 Semantic 检查前展开；为实现“目标显式成员优先”，静态 wrapper 候选在进入普通成员表前需要判断是否与目标显式成员构成声明冲突。该预检必须复用与普通手写成员相同的声明冲突判定：只有普通声明规则本就禁止的冲突才跳过 wrapper；调用侧可消歧的潜在重叠必须生成 wrapper，再由后续普通成员重载决议处理。实例 wrapper 和不同来源的静态 wrapper 不适用目标显式成员优先，生成后直接交由普通声明检查处理。
...
- [ ] 调整 `@mixable` 展开阶段的目标显式成员冲突预检，使其复用统一声明冲突判定；wrapper 生成后不增加独立重载决议逻辑。
