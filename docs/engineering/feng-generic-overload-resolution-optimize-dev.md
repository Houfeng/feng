...
- `@mixable` wrapper 在完整 Semantic 检查前展开并进入普通成员表，此后不存在独立的
  重载决议路径，自然使用普通静态方法或实例方法的规则；生成前的目标显式优先另按
  [mixin `@mixable` 目标显式优先优化](./feng-mixin-optimize.md)检查实例投影冲突。
...
适用性和匹配类别应在同一次候选探测中共享推导、替换和类型关系结果。顶层函数、模块公开函数、普通成员方法、静态方法和 `fit` 方法必须复用同一比较逻辑。

`@mixable` 不增加候选探测或候选比较入口。静态 wrapper 和实例 wrapper 均在完整 Semantic 检查前生成为普通成员，后续直接经过上述普通方法路径；不得按 `mixable` 来源附加或改变匹配类别。
...
最后一项不是独立的 `@mixable` 重载决议。`@mixable` 在完整 Semantic 检查前展开；静态
wrapper 候选进入普通成员表前，按 [mixin `@mixable` 目标显式优先优化](./feng-mixin-optimize.md)
执行两项预检：继续检查完整静态成员冲突；目标显式成员也是 `@mixable` 时，额外检查
双方删除首参数后的实例投影冲突。实例投影检查只稳定静态方法生成实例 wrapper 时的
固有签名变化，不参与调用点候选比较。实例 wrapper 和不同来源的静态 wrapper 不适用
目标显式优先，生成后直接交由普通声明检查处理。
...
- [ ] 改造全部顶层函数、普通成员方法、静态方法和 `fit` 方法的重载入口，移除整体 `match_priority` 依赖，并保证本地声明与跨包恢复声明共享同一候选比较实现。
- [ ] 调整 `@mixable` 展开阶段的目标显式成员冲突预检：完整静态成员冲突复用统一声明
  冲突判定；目标显式 `@mixable` 的实例投影冲突遵循
  [专项设计](./feng-mixin-optimize.md)；wrapper 生成后不增加独立重载决议逻辑。
