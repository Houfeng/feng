...
### P01：新增用例触发尚未支持的具体静态方法值

- 状态：已完成（统一方法值专项 MV03 已恢复验收）
- 发现阶段：Semantic 正向测试
...
- 原因分析：friend 专项实施当时，Feng 已支持的具体成员方法值范围包含实例方法，不包含
  该静态方法值形式；该次修复不能借 friend 测试扩大独立的方法值能力。
- 决策：正向用例保留实例方法值，并分别在字段初始化、构造函数和终结器中通过直接调用
  覆盖静态 friend 方法；具体静态方法值与其他尚缺方法值统一由
  [Feng 成员方法值补齐开发草案](./feng-object-form-spec-method-value-dev.md) 设计和实施，
  本文不重复定义其语义。
- 解决：已移除新增测试中的静态方法值断言，保留实例方法值和静态方法直接调用覆盖。
- 恢复验收：已由
  [Feng 成员方法值补齐开发草案 MV03](./feng-object-form-spec-method-value-dev.md#mv03-独立交付记录)
  恢复字段初始化、构造函数和终结器中的 `Vault.readShared` 方法值用例；实现复用既有
  friend 类型实现上下文授权，没有增加 friend 专用方法值路径。专项及全量验证结果以
  MV03 独立交付记录为准。
- 验证：本专项原有 Semantic 验证通过；静态方法值恢复后的 Semantic、FCTS 与沙箱外
  完整 `make test` 均通过，详见上述 MV03 交付记录。
