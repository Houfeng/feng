...
- `completion` 以当前位置可见的局部名、模块级声明、导入模块公开名和对象成员为范围,不要求依赖额外构建步骤; 对编辑过程中常见的临时不完整成员访问,例如刚输入 `.` 或成员名前缀尚未解析通过时,应尽量返回可推断的对象成员补全; 当对象来自局部变量时,显式类型标注与对象字面量初始化式均可作为对象类型推断来源。
