...
- `@bounded`、`@abi`、`extern func` 调用方式与原生库来源等编译期语义,都必须以 `.ft` 中的声明事实与元信息表达,而不是依赖文本接口源码或包内 `feng.fm`。
...
1. 编译器扫描所有 `.ff` 源文件,按 `module` 声明分组,将同一模块的全部公开声明合并
2. 在语义分析成功后按模块名层级导出每个公开模块的 `.ft` 文件,放入 `build/mod/` 目录
3. 将 feng 普通 `type` / `func` / 模块级 `let` / `var` 实现编译为 C → 生成静态库,放入 `lib/` 对应平台子目录（若 `abi` 含 `feng`）
4. 汇总公开 `extern func` 导入声明并保留其原生库来源与调用方式元信息; 需要随包分发的依赖型原生库按平台放入 `build/extlib/<platform>/`
...
3. 由 `.ft` 读取器把公开包表解析为统一查询视图,将公开 `type`、公开 `enum`、公开顶层 `func`、公开模块级 `let` / `var`、公开成员、`spec` / `fit` 与 `@bounded` 相关事实引入当前编译期查询环境
    对公开泛型声明,该查询视图还必须暴露类型参数顺序、约束目标、未实例化签名骨架以及泛型父 `spec` / 泛型 `fit` 使用事实,使使用方只依赖 `.ft` 即可完成跨包泛型语义分析
4. 根据声明关键字与 `extern` 元信息,编译器直接在 `.fb` 中定位当前平台下实际存在的 `lib/` 正式库文件; 若所需文件不存在则报错; 对公开 `extern func` 导入声明,编译器先按 `.ft` 保留的原生库名尝试匹配同包或其他依赖包 `extlib/<当前平台>/` 下的主机静态库文件（Linux / macOS `lib<name>.a`,Windows `<name>.lib`）,命中时直接提取该静态库参与链接,未命中时再回退为常规原生库链接参数
...
2. `target=bin` 构建阶段先按公开 `extern func` 元信息把 `extlib/` 下命中的静态库接入链接,并仅对同样由这些 `extern func` 元信息命中的动态库执行释放到可执行文件同目录; 未命中的 `extlib/` 制品保持不参与
...
- `extern func` 属于导入型外部函数声明,生成的公开 `.ft` 必须保留其带参 `@cdecl` / `@stdcall` / `@fastcall` 元信息,以便使用方无需手写这些注解即可完成编译与链接
...
- `extlib/` 仅用于依赖型原生库分发; 它不参与模块解析、类型检查与 `lib/` 正式库链接推导,且只有在公开 `extern func` 元信息显式引用对应库名时,其中命中的静态库才可参与链接、命中的动态库才可参与运行期释放
