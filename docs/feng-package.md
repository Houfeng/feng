...
- `use` 仍然以模块名为导入目标; 编译器先按模块名定位对应 `.fi`,再解析其公有声明,并将该模块公开 `type` 与公开顶层 `fn` 按模块导入规则引入当前模块作用域,最后链接包内对应库文件。
...
2. Feng 使用方: `use` 公开模块名后即可直接调用 `extern` 接口,编译器通过包内固定 `mod/` 目录定位对应 `.fi`; 使用方源码无需额外手写带参 `@cdecl` / `@stdcall` / `@fastcall`,因为包内接口元信息已包含对应原生库来源与调用方式
...
- 若公开 `extern fn` 属于导入型外部函数声明,生成的接口文件必须保留其带参 `@cdecl` / `@stdcall` / `@fastcall` 元信息,以便使用方在无需手写这些注解的情况下完成编译与链接
...
// 导入 C ABI 兼容包(.fcp),无需额外手写带参调用方式注解
use c_mylib;
```
