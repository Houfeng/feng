...
- 公开 `.ft` 只保留公开声明事实、签名、类型、ABI 元信息以及 type 实例成员绑定推断事实,不保留函数体、语句、表达式、初始化表达式、常量值、运行时数据或私有模块级声明。公开 `type` 的字段声明属于对象布局与泛型实例化所需 ABI 元信息,即使字段本身不是公开可访问成员,也必须随该公开 `type` 写入 `.ft`。
