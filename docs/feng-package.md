...
`.fb` 包当前只定义 feng 层正式库分发（`lib/`）; `extern fn` 的原生库需求通过公开 `.ft` 元信息表达,必要时可由 `extlib/` 随包携带对应制品。所有模式均支持闭源分发、压缩归档与编译加速,不破坏 feng → C 编译架构。
...
- `[assets]` 节声明开发态资源复制配置; 省略时为空,仅开发阶段有效,不出现在分发包内。节内每个键是目标目录（相对可执行文件目录或 `.fb` 根目录）,每个值是源目录路径（相对 `feng.fm` 所在目录）且不得为空字符串。`target=bin` 时将指定资源目录复制到可执行文件同级目标目录; `target=lib` 时普通目标目录会复制到 `build/assets/` staging 后写入 `.fb` 内对应目标目录,但当目标目录精确为 `extlib` 时,为复用 `.fb` 顶层 `extlib/` 能力目录,构建阶段直接复制到 `build/extlib/`,打包时写入 `.fb/extlib/`,不额外插入 `assets/` 目录层。构建工具写回 `feng.fm` 时保留 `[assets]` 的声明顺序。
...
5. 生成 `feng.fm`,并将 `build/mod/**/*.ft`、对应平台正式库文件、可选 `build/extlib/` 目录树（包含构建阶段收集的依赖型原生库,以及 `[assets].extlib` 直接 staging 的内容）以及 `build/assets/` 中其余 `[assets]` staging 目录打包为 `.fb`
...
4. 根据声明关键字与 `extern` 元信息,编译器直接在 `.fb` 中定位当前平台下实际存在的 `lib/` 正式库文件; 若所需文件不存在则报错; 对公开 `extern fn` 导入声明,编译器先按 `.ft` 保留的原生库名尝试匹配同包或其他依赖包 `extlib/<当前平台>/` 下的主机静态库文件（Linux / macOS `lib<name>.a`,Windows `<name>.lib`）,命中时直接提取该静态库参与链接,未命中时再回退为常规原生库链接参数
...
2. `target=bin` 构建阶段先按公开 `extern fn` 元信息把 `extlib/` 下命中的静态库接入链接,并仅对同样由这些 `extern fn` 元信息命中的动态库执行释放到可执行文件同目录; 未命中的 `extlib/` 制品保持不参与
...
- `extern fn` 属于导入型外部函数声明,生成的公开 `.ft` 必须保留其带参 `@cdecl` / `@stdcall` / `@fastcall` 元信息,以便使用方无需手写这些注解即可完成编译与链接
...
- `extlib/` 仅用于依赖型原生库分发; 它不参与模块解析、类型检查与 `lib/` 正式库链接推导,且只有在公开 `extern fn` 元信息显式引用对应库名时,其中命中的静态库才可参与链接、命中的动态库才可参与运行期释放
