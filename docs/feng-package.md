...
- `feng.fm` 采用固定节名的简洁文本格式: 以 `[section]` 声明节,节内以 `key: "value"` 声明字段值。当前标准节为 `[package]` 与 `[dependencies]`。`[package]` 保存包与项目字段,`[dependencies]` 保存直接依赖列表。
...
- `[dependencies]` 节中的每个键表示一个直接依赖包名,值为该依赖的精确版本字符串; 同一依赖包名不得重复出现。
