...
- `cdeps/`: 可选,携带需要随包分发的依赖型原生库制品
- `复制资源目录`: 可选,携带项目声明的运行时资源文件（目录由 `assets` / `copy` 配置指定）

`.fb` 包当前只定义 feng 层正式库分发（`lib/`）; `extern fn` 的原生库需求通过公开 `.ft` 元信息表达,必要时可由 `cdeps/` 随包携带对应制品。所有模式均支持闭源分发、压缩归档与编译加速,不破坏 feng → C 编译架构。
...
│       └── libdep.dylib
│
├── <configured-dir>/        // 可选③：资源目录（由开发态 assets/copy 配置指定）
│   └── ...
```
...
- `assets` / `copy` 字段声明开发态资源复制配置; 省略时为空,两者不得同时出现; 仅开发阶段有效,不出现在分发包内。`target=bin` 时将指定资源复制到可执行文件同目录,`target=lib` 时将指定资源复制并写入 `.fb` 内配置指定目录（相对 `.fb` 根目录）。
...
4. 汇总公开 `extern fn` 导入声明并保留其原生库来源与调用方式元信息; 需要随包分发的依赖型原生库按平台写入 `cdeps/`
5. 生成 `feng.fm`,并将 `build/mod/**/*.ft`、对应平台正式库文件以及 `assets` / `copy` 指定资源打包为 `.fb`
...
4. 根据声明关键字与 `extern` 元信息,编译器直接在 `.fb` 中定位当前平台下实际存在的 `lib/` 正式库文件; 若所需文件不存在则报错; 公开 `extern fn` 导入声明则补充所需原生库链接信息
5. 使用方无需手写带参 `@cdecl` / `@stdcall` / `@fastcall`,包内公开 `.ft` 已携带对应原生库来源与调用方式元信息

### 6.3 依赖型原生库分发流程

1. 若包需随分发携带依赖型原生库,按平台放入 `cdeps/`
2. `target=bin` 构建阶段按运行时需要释放 `cdeps/` 下动态库到可执行文件同目录
...
- `cdeps/` 仅用于依赖型原生库分发,不参与模块解析、类型检查与 `lib/` 正式库链接推导
