...
- 公开 `.ft` 只保留公开声明事实、签名、类型、ABI 元信息以及 `@bounded` 相关语义事实,不保留函数体、语句、表达式、初始化表达式、常量值、运行时数据或私有模块级声明。公开 `type` 的字段声明属于对象布局与泛型实例化所需 ABI 元信息,即使字段本身不是公开可访问成员,也必须随该公开 `type` 写入 `.ft`。
- 若模块公开了泛型 `type`、`spec`、顶层 `fn` 或成员方法,则写入 `.fb/mod/**/*.ft` 的必须是其声明级泛型事实: 类型参数、约束、未实例化签名骨架、父 `spec` / `fit` 的泛型使用关系；不得要求使用方重新读取 provider 源码或依赖包内某个已单态化实例才能消费这些声明。
...
- `@bounded`、`@abi`、`extern fn` 调用方式与原生库来源等编译期语义,都必须以 `.ft` 中的声明事实与元信息表达,而不是依赖文本接口源码或包内 `feng.fm`。
...
4. 将公开的 `@abi` 接口编译为 C ABI 兼容代码 → 生成动态库/静态库和 C 头文件,放入 `clib/` 与 `include/`（若 `abi` 含 `c`）; 公开 `extern fn` 导入声明则保留其原生库来源与调用方式元信息
...
3. 由 `.ft` 读取器把公开包表解析为统一查询视图,将公开 `type`、公开 `enum`、公开顶层 `fn`、公开模块级 `let` / `var`、公开成员、`spec` / `fit` 与 `@bounded` 相关事实引入当前编译期查询环境
    对公开泛型声明,该查询视图还必须暴露类型参数顺序、约束目标、未实例化签名骨架以及泛型父 `spec` / 泛型 `fit` 使用事实,使使用方只依赖 `.ft` 即可完成跨包泛型语义分析
4. 根据声明关键字与 `@abi` / `extern` 元信息,编译器直接在 `.fb` 中定位当前平台下实际存在的 `lib/` 或 `clib/` 正式库文件; 若所需文件不存在则报错; 公开 `extern fn` 导入声明则补充所需原生库链接信息
...
- C ABI 层的直接布局仍仅支持 ABI 稳定类型; 公开的可调用 ABI 接口对 `string` 的使用统一遵循 [Feng 内建类型规范](./feng-builtin-type.md) 中的桥接规则, 对数组的使用统一遵循 [Feng 数组与标准库数组规范](./feng-std-array.md) 中的桥接规则; 进入 ABI 边界的 `@abi fn` 禁止捕获变量、使用闭包
- 公开导出到 `clib/` 的接口以公开 `@abi` 声明为准,头文件内容由其自动生成; 若公开 `extern fn` 属于导入型外部函数声明,生成的公开 `.ft` 必须保留其带参 `@cdecl` / `@stdcall` / `@fastcall` 元信息,以便使用方无需手写这些注解即可完成编译与链接
