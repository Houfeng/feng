...
feng 编译器在消费 `.fb` 时,主要直接读取 `mod/` 下的 `.fi` 与对应平台下的 `lib/` / `clib/` 库文件; 在需要判断平台/架构与能力层时,也可直接读取包内 `feng.fm`。
...
- 开发项目使用的 `feng.fm` 与分发包内的 `feng.fm` 共用文件名和基础键值格式,但职责不同: 前者服务于项目构建,后者服务于 `.fb` 分发元信息表达; feng 编译器不读取项目 `feng.fm`,但在消费 `.fb` 时可按需读取包内 `feng.fm`。
...
- `arch` 字段声明分发包内实际携带制品的平台/架构集合,采用逗号分隔的标识列表（例如 `linux-x64,windows-x64,macos-arm64`）; 该字段由构建工具在打包时填写,开发项目可省略; feng 编译器在消费 `.fb` 时可读取该字段以判断当前平台是否存在可用制品。
- `abi` 字段声明本包携带哪些能力层,取值为 `feng`、`c` 或 `feng,c`; `feng` 表示存在 `lib/` 目录,`c` 表示存在 `clib/` 与 `include/` 目录; 该字段必须与包内实际目录结构一致,声明与实际不符则该包非法; 开发项目若不作为包发布可省略此字段; feng 编译器在消费 `.fb` 时可读取该字段以判断应从包内读取哪一层产物。
...
1. 编译器打开目标 `.fb`,必要时直接读取包内 `feng.fm` 的 `arch`、`abi` 等分发元信息,判断当前平台是否存在可用制品
2. `use` 公开模块名 → 编译器按模块名推导路径,定位 `mod/` 下对应 `.fi`
3. 解析 `.fi` 中公有声明及其注解元信息,将公开 `type`、公开顶层 `fn` 与公开模块级 `let` / `var` 引入当前模块作用域
4. 根据声明关键字与 `@fixed` / `extern` 元信息,并结合包内 `feng.fm` 的 `arch` / `abi`,从 `.fb` 中直接定位并读取当前平台下的 `lib/` 或 `clib/` 文件; 公开 `extern fn` 导入声明则补充所需原生库链接信息
