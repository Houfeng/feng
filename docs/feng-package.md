...
feng 编译器在消费 `.fb` 时,只直接读取 `mod/` 下的 `.fi` 与对应平台下的正式库文件; 不读取包内 `feng.fm`,也不依赖 `feng.fm` 判断制品是否存在。
...
- 正式分发面只包含 `.fi` 与库文件,不包含 `.o` / `.obj` 这类中间目标文件
...
2. `use` 公开模块名 → 编译器按模块名推导路径,定位 `mod/` 下对应 `.fi`
3. 解析 `.fi` 中公有声明及其注解元信息,将公开 `type`、公开顶层 `fn` 与公开模块级 `let` / `var` 引入当前模块作用域
4. 根据声明关键字与 `@fixed` / `extern` 元信息,编译器直接在 `.fb` 中定位当前平台下实际存在的 `lib/` 或 `clib/` 正式库文件; 若所需文件不存在则报错; 公开 `extern fn` 导入声明则补充所需原生库链接信息
5. 使用方无需手写带参 `@cdecl` / `@stdcall` / `@fastcall`,包内 `.fi` 已携带对应原生库来源与调用方式元信息
...
- feng 编译器只消费 `.fi` 与实际存在的正式库文件,不读取 `feng.fm`; `feng.fm` 用于构建、依赖管理与打包校验
