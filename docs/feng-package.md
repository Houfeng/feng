...
- `[assets]` 节声明开发态资源复制配置; 省略时为空,仅开发阶段有效,不出现在分发包内。节内每个键是目标目录（相对可执行文件目录或 `.fb` 根目录）,每个值是源路径（相对 `feng.fm` 所在目录）。`target=bin` 时将指定资源复制到可执行文件同级目标目录,`target=lib` 时将指定资源复制并写入 `.fb` 内对应目标目录。
