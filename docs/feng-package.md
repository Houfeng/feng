...
- 公开 `.ft` 只保留公开声明事实、签名、类型、ABI 元信息、必要的 `extern` 链接事实以及 type 实例成员绑定推断事实,不保留函数体、语句、表达式、初始化表达式、常量值、运行时数据或普通私有模块级声明。公开 `type` 的字段声明属于对象布局与泛型实例化所需 ABI 元信息,即使字段本身不是公开可访问成员,也必须随该公开 `type` 写入 `.ft`。
...
- 私有成员（`seal`）与私有模块级绑定不写入接口文件,完全隐藏
