...
公开接口描述文件使用 `.fi` 扩展名,由 feng 编译器自动生成,包含公有签名以及编译期语义检查所需的初始化元信息,但不包含实现逻辑:
...
- 对公开 `let` 成员,接口元信息必须体现该成员是否已在类型声明初值阶段完成显式绑定。
- 对公开构造函数重载,接口元信息必须体现其参数列表,并指明哪些公开 `let` 成员会在该构造函数阶段完成显式绑定。
- 这些元信息用于支持包使用方在编译期检查 `let` 成员是否发生重复绑定; 编译器可采用注解、附加元数据段或其他等价序列化形式,不要求暴露具体实现表达式。

### 3.6 编译与使用流程

1. 发布方: feng 源码 → 编译为 C → 生成 C 静态库 → 自动生成 `api.fi` / `feng.fm` → 打包为 `.fp`
...
- 若包中公开 `type` 包含 `let` 成员,则编译产物必须携带其显式绑定状态相关元信息,以保证使用方仍可在编译期完成对象字面量与构造路径的重复绑定检查
...
2. Feng 使用方: `use` 模块名后即可直接调用 `extern` 接口,无需额外声明 `link`,因为包内已包含对应链接声明
