...
1. 若包需随分发携带依赖型原生库,按平台放入 `extlib/`
2. `target=bin` 构建阶段按运行时需要释放 `extlib/` 下动态库到可执行文件同目录
...
- `extlib/` 仅用于依赖型原生库分发,不参与模块解析、类型检查与 `lib/` 正式库链接推导
