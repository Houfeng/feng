...
- `extern fn` 是语言中的统一外部函数声明语法,必须无函数体; 本文档只定义它在 C ABI 路径下的导入规则。
...
- C ABI 路径下,带类型参数的 `extern fn` 仅在每个参数位与返回位抹除类型参数后仍对应唯一且 ABI-stable 的 C surface 时才合法; 若某一位置会随具体类型实参改变 C surface,则编译器必须拒绝该声明。
