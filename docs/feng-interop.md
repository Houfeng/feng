...
本文档用于补充 [feng-language.md](./feng-language.md) 中的 C 互操作概要说明,聚焦 FC 语言与 C 库链接、C 兼容类型、外部函数声明和回调函数定义规则。

## 1 C互操作概览

- FC 通过 `link`、`extern type` 和 `extern fn` 实现与 C 语言的安全互操作。
- `link` 负责链接 C 标准库、第三方 C 库或 FC 编译二进制产物。
- `extern type` 用于定义 C 兼容结构体和 C 函数指针类型。
- `extern fn` 用于声明 C 外部函数以及定义可传给 C 的 FC 回调函数。

## 2 C库链接指令(link)

使用 `link "库名/路径"` 指令链接 C 库或 FC 编译二进制库。`link` 声明必须位于 `mod` / `use` 段之后、其他语句之前; 若文件中存在 `use` 导入段,则必须位于连续 `use` 段之后; 若不存在 `use`,则位于 `mod` 声明之后。编译器自动识别链接类型,支持三种形式:
