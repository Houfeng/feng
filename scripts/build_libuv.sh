...
if [[ ! -d "${TARGET_DIR}" ]]; then
  ...
  echo "error: run ${PROJECT_ROOT}/scripts/fetch_libuv.sh first" >&2
  exit 1
fi

if [[ ! -f "${TARGET_DIR}/Makefile" ]]; then
  echo "error: ${TARGET_DIR}/Makefile is missing" >&2
  echo "error: run ${PROJECT_ROOT}/scripts/fetch_libuv.sh again to regenerate the vendored build files" >&2
  exit 1
fi

echo "==> Building libuv into ${OUTPUT_DIR}"
"${MAKE_BIN}" -C "${TARGET_DIR}" OUTPUT_DIR="${OUTPUT_DIR}" install
echo "==> Built ${OUTPUT_DIR}/libfeng_std_uv.a"
