...
static void feng_finalize_managed(FengManagedHeader *header) {
     ...
     * constructed object (including managed children). */
    if (header->desc != NULL && header->desc->finalizer != NULL) {
        header->desc->finalizer(self);
    }

    /* Tag-specific child release. Object-tagged values rely entirely on the
     * user finalizer (which codegen emits per-type to release each managed
     * field) — there is no descriptor-driven field walk in Phase 1A. */
    switch (header->tag) {
